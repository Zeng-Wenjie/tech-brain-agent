 ...
 * ToolServiceChainAnalyzer / SseEventChainAnalyzer / CodeExplanationGenerator -> 包装统一 code_analysis 外层 JSON 返回给 finalAnswer 和 tool_call_log。</p>
 *
 * <p>边界说明：本 Tool 只做静态只读分析，不修改项目文件，不访问 workspace 外路径，不接入 RAG/Milvus/向量化，
 * 已支持 P5.6 代码说明；不实现 P5.7 风险分析、P5.8 测试步骤或 P5.9 开发日志保存。</p>
 */
...
public class AnalyzeCodeTool extends AbstractAiTool { // 统一项目代码分析工具。
    ...
    public String description() { // 给模型看的统一工具描述。
        return "统一项目代码分析工具。支持单文件结构分析、普通调用链分析、Controller 到 Service 调用链分析、Tool 到业务 Service 调用链分析、前后端 SSE 事件链路分析和代码说明生成。根据 analysisType 分发到内部分析器。只做静态只读分析，不修改项目文件。";
    }
    ...
    public ObjectNode parametersSchema() { // 统一 analyzeCode 参数 schema。
        ...
        addProperty(schema, "analysisType", createStringProperty("分析类型，可选。AUTO、STRUCTURE、CALL_CHAIN、CONTROLLER_SERVICE、TOOL_SERVICE、SSE_EVENT_CHAIN、EXPLANATION。默认 AUTO"), false); // 分析类型。
        ...
        includeSnippetProperty.put("description", "是否包含少量代码片段，可选；结构/链路分析默认 true，EXPLANATION 默认 false"); // 片段说明。
    ...
    private void normalizeClassNameFallback(ObjectNode arguments, CodeAnalysisType analysisType) { // 将 className 转成旧分析器可理解的 path 兜底。
        ...
        if (analysisType == CodeAnalysisType.STRUCTURE
                ...
                || analysisType == CodeAnalysisType.EXPLANATION) { // 这些分析器通过 path 字段支持类名唯一定位。
    ...
    private void applyIncludeSnippetDefault(ObjectNode arguments, CodeAnalysisType analysisType) { // 根据分析类型补齐 includeSnippet 默认值。
        ...
        arguments.put("includeSnippet", analysisType != CodeAnalysisType.EXPLANATION); // 说明生成默认 false，其它分析默认 true。
    }
