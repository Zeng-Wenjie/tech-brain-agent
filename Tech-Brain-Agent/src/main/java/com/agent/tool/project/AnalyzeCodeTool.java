...
import com.agent.entity.dto.DevActionLogSaveCommand; // P5.9 开发日志保存入参。
import com.agent.service.DevActionLogService; // P5.9 开发日志服务，保存代码分析结果。
 ...
 * 已支持 P5.6 代码说明、P5.7 风险点说明和 P5.8 测试步骤生成；P5.9 支持 saveToDevLog=true 时把本次分析结果保存到
 * 开发日志（dev_action_log），这是分析执行成功后的附加动作，不是新的 analysisType，保存失败不影响分析主流程。</p>
 */
...
public class AnalyzeCodeTool extends AbstractAiTool { // 统一项目代码分析工具。
    ...
    private final DevActionLogService devActionLogService; // P5.9 开发日志服务，saveToDevLog=true 时保存分析结果。

    public AnalyzeCodeTool(List<CodeAnalysisHandler> handlers,
                           DevActionLogService devActionLogService) { // Spring 注入所有内部 Analyzer 和开发日志服务。
        this.handlerMap = buildHandlerMap(handlers); // 构造只按枚举分发的映射。
        this.devActionLogService = devActionLogService; // 保存开发日志服务，用于 P5.9 附加保存动作。
    }
    ...
    public ObjectNode parametersSchema() { // 统一 analyzeCode 参数 schema。
        ...
        ObjectNode saveToDevLogProperty = objectMapper.createObjectNode(); // P5.9 boolean 字段手动创建。
        saveToDevLogProperty.put("type", "boolean"); // boolean 类型。
        saveToDevLogProperty.put("description", "是否将本次分析结果保存到开发日志，可选，默认 false"); // 字段说明。
        addProperty(schema, "saveToDevLog", saveToDevLogProperty, false); // 开发日志保存开关。
        addProperty(schema, "devLogTitle", createStringProperty("可选，开发日志标题。为空时根据 analysisType 和目标自动生成"), false); // 开发日志标题。
        addProperty(schema, "devLogSummary", createStringProperty("可选，开发日志摘要。为空时从分析结果自动提取"), false); // 开发日志摘要。
        ObjectNode devLogTagsProperty = objectMapper.createObjectNode(); // P5.9 数组字段手动构造。
        devLogTagsProperty.put("type", "array"); // 数组类型。
        ObjectNode devLogTagsItems = objectMapper.createObjectNode(); // 数组元素类型。
        devLogTagsItems.put("type", "string"); // 元素为字符串。
        devLogTagsProperty.set("items", devLogTagsItems); // 设置元素类型。
        devLogTagsProperty.put("description", "可选，开发日志标签，例如 project、analysis、risk、test"); // 字段说明。
        addProperty(schema, "devLogTags", devLogTagsProperty, false); // 开发日志标签。
    ...
    public String execute(JsonNode arguments) { // 统一分析执行入口。
        ...
        try {
            ...
            ObjectNode unifiedResult = buildUnifiedResult(effectiveType, normalizedArguments, innerResult); // 包装统一外层。
            saveToDevLogIfRequested(effectiveType, normalizedArguments, unifiedResult); // P5.9：saveToDevLog=true 且分析成功时保存到开发日志。
            return unifiedResult.toString(); // 返回统一外层结果（含 devLogSaved 字段）。
        } catch (Exception e) {
    ...
    private void saveToDevLogIfRequested(CodeAnalysisType analysisType,
                                         ObjectNode arguments,
                                         ObjectNode unifiedResult) { // P5.9：按需把本次分析结果保存到开发日志。
        if (!arguments.path("saveToDevLog").asBoolean(false)) { // 未要求保存时直接返回。
            return; // 不附加任何 devLog 字段，保持原有返回结构。
        }
        if (!unifiedResult.path("success").asBoolean(false)) { // 分析失败时不保存，避免落库无效结果。
            unifiedResult.put("devLogSaved", false); // 标记未保存。
            addWarning(unifiedResult, "本次分析未成功，未保存到开发日志。"); // 进入 warnings 提示。
            return; // 失败不保存。
        }
        if (devActionLogService == null) { // 极端情况下服务未注入。
            unifiedResult.put("devLogSaved", false); // 标记未保存。
            unifiedResult.put("devLogError", "保存开发日志失败，但分析结果已生成。"); // 写入失败原因。
            addWarning(unifiedResult, "保存开发日志失败，不影响本次分析结果。"); // 进入 warnings。
            return; // 不保存。
        }
        try {
            DevActionLogSaveCommand command = buildDevLogCommand(analysisType, arguments, unifiedResult); // 构造保存入参。
            Long devLogId = devActionLogService.saveCodeAnalysisLog(command); // 保存到 dev_action_log。
            if (devLogId != null) { // 保存成功。
                unifiedResult.put("devLogSaved", true); // 标记已保存。
                unifiedResult.put("devLogId", devLogId); // 写入开发日志 ID，tool_call_log.result_json 会一并记录。
            } else { // 保存失败（已在 Service 内兜底为 null）。
                unifiedResult.put("devLogSaved", false); // 标记未保存。
                unifiedResult.put("devLogError", "保存开发日志失败，但分析结果已生成。"); // 写入失败原因。
                addWarning(unifiedResult, "保存开发日志失败，不影响本次分析结果。"); // 进入 warnings。
            }
        } catch (Exception e) { // 兜底：保存异常绝不影响分析主流程。
            log.warn("[AnalyzeCodeTool] save dev action log failed, analysisType: {}, error: {}", analysisType, e.getMessage()); // 不打印 result_json。
            unifiedResult.put("devLogSaved", false); // 标记未保存。
            unifiedResult.put("devLogError", "保存开发日志失败，但分析结果已生成。"); // 写入失败原因。
            addWarning(unifiedResult, "保存开发日志失败，不影响本次分析结果。"); // 进入 warnings。
        }
    ...
    private DevActionLogSaveCommand buildDevLogCommand(CodeAnalysisType analysisType,
                                                       ObjectNode arguments,
                                                       ObjectNode unifiedResult) { // 由分析上下文与统一结果构造开发日志保存入参。
        ToolCallingRequestContext context = ToolCallingContextHolder.get(); // 读取当前请求上下文（userId/conversationId/traceId）。
        JsonNode target = unifiedResult.path("target"); // 统一结果中的目标信息。
        DevActionLogSaveCommand command = new DevActionLogSaveCommand(); // 构造保存入参。
        command.setUserId(context == null ? null : context.getUserId()); // 当前用户。
        command.setConversationId(context == null ? null : context.getConversationId()); // 当前会话。
        command.setTraceId(context == null ? null : context.getTraceId()); // 与 tool_call_log 一致的 traceId，用于关联。
        command.setAnalysisType(analysisType == null ? null : analysisType.name()); // analysisType。
        command.setTargetPath(textOrNull(target, "path")); // workspace 相对路径。
        command.setClassName(textOrNull(target, "className")); // 类名。
        command.setMethodName(textOrNull(target, "methodName")); // 方法名。
        command.setEndpoint(textOrNull(target, "endpoint")); // 接口路径。
        command.setToolName(textOrNull(target, "toolName")); // Tool 名称。
        command.setEventName(textOrNull(target, "eventName")); // SSE 事件名。
        command.setDevLogTitle(textOrNull(arguments, "devLogTitle")); // 可选标题，为空时 Service 自动生成。
        command.setDevLogSummary(textOrNull(arguments, "devLogSummary")); // 可选摘要，为空时 Service 自动提取。
        command.setResultJson(unifiedResult.toString()); // 完整统一分析结果 JSON（已脱敏、相对路径）。
        return command; // 返回保存入参。
    }

    private void addWarning(ObjectNode unifiedResult, String warning) { // 向统一结果的 warnings 追加一条提示。
        JsonNode warningsNode = unifiedResult.path("warnings"); // 读取已有 warnings。
        if (warningsNode instanceof ArrayNode warnings) { // warnings 已是数组。
            warnings.add(warning); // 直接追加。
        } else { // 缺失时新建数组。
            ArrayNode warnings = objectMapper.createArrayNode(); // 创建 warnings 数组。
            warnings.add(warning); // 追加提示。
            unifiedResult.set("warnings", warnings); // 写回结果。
        }
    }

    private String textOrNull(JsonNode node, String fieldName) { // 读取 JSON 文本字段，空白返回 null。
        if (node == null) { // 空节点。
            return null; // 返回 null。
        }
        String value = node.path(fieldName).asText(""); // 读取字段。
        return value == null || value.isBlank() ? null : value; // 空白返回 null。
    }
}
