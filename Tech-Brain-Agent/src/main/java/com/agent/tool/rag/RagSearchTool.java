 ...
 * <p>调用链为：ToolCallingChatServiceImpl解析模型tool_call -> ToolRegistry根据工具名找到RagSearchTool -> execute(arguments)解析query -> AgentService.searchRagSegments(query)检索Milvus -> 拼接tool result返回给第二次DeepSeek调用。</p>
...
public class RagSearchTool extends AbstractAiTool { // 具体业务工具继承公共抽象类，复用参数读取和Schema构造方法。
    ...
    public String execute(JsonNode arguments) { // 执行模型请求的ragSearch工具。
        ...
        log.debug("[ToolChat] real rag result length: {}", result == null ? 0 : result.length()); // RAG结果可能包含正文，只打印长度。
        return result; // 返回给ToolCallingChatServiceImpl，随后作为tool message发送给DeepSeek。
    }
