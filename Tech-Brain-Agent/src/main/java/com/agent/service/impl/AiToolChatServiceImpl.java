...
import com.agent.service.AgentService; // 注入已有 AgentService，复用原 Milvus RAG 检索逻辑。
...
import org.springframework.beans.factory.annotation.Autowired; // 注入已有 RAG Service。
...
import java.util.List; // 接收真实 RAG 检索返回的多个知识片段。
import javax.net.ssl.SSLHandshakeException; // 识别 DeepSeek HTTPS 握手失败，方便定位网络/代理问题。
...
public class AiToolChatServiceImpl implements AiToolChatService { // 最小 Tool Calling 闭环实现。
    ...
    @Value("${milvus.collection-name:article_vector}") // 读取 Milvus collection 名称，用于 ToolChat 检索日志。
    private String milvusCollectionName; // 当前项目文章向量所在的 Milvus collection。

    @Autowired // 注入已有 AgentService，避免在 ToolChat 中重写 Milvus 底层搜索代码。
    private AgentService agentService; // realRagSearch 会调用 AgentService.searchRagContents(query)。

    private final ObjectMapper objectMapper = new ObjectMapper(); // 使用本地 JSON 解析器，避免 IDE 找不到 ObjectMapper Bean 报红。

    private final HttpClient httpClient = HttpClient.newBuilder() // 只给 /api/ai/tool-chat 使用的最小 HTTP 客户端。
            .connectTimeout(Duration.ofSeconds(30)) // 明确连接超时，避免网络不通时长时间卡住。
            .version(HttpClient.Version.HTTP_1_1) // 强制 HTTP/1.1，规避部分代理/VPN 对 HTTP/2 TLS 握手兼容问题。
            .build(); // 构建 ToolChat 专用 HttpClient。
    ...
    public ToolChatResponse toolChat(ToolChatRequest request) { // 执行 POST /api/ai/tool-chat 的完整流程。
        ...
        String ragResult = realRagSearch(query); // 调用真实 RAG 检索，将 Milvus 命中的知识片段作为 tool result。
    ...
    private ObjectNode buildSecondRequest(String userMessage, JsonNode firstMessage, String toolCallId, String ragResult) { // 构造第二次 DeepSeek 请求。
        ...
        toolMessage.put("content", ragResult); // 工具执行结果是真实 RAG 检索结果或无结果提示。
    ...
    private JsonNode callDeepSeek(ObjectNode requestBody) { // ToolChat 专用 DeepSeek HTTP 调用。
        try { // 捕获网络、序列化和解析异常。
            ...
            String requestUrl = buildChatCompletionsUrl(); // 统一生成最终请求地址，避免重复拼接 v1。
            log.info("[ToolChat] DeepSeek request url: {}", requestUrl); // 打印最终请求 URL，确认是 /v1/chat/completions。
            log.info("[ToolChat] DeepSeek model: {}", requestBody.path("model").asText()); // 打印本次请求模型，确认仍为 deepseek-v4-pro。
            log.info("[ToolChat] DeepSeek thinking: {}", requestBody.path("thinking").path("type").asText()); // 打印 thinking 配置，确认 disabled 已带上。
            HttpRequest httpRequest = HttpRequest.newBuilder(URI.create(requestUrl)) // 构造 POST /chat/completions 请求。
            ...
            log.error("[ToolChat] DeepSeek IO error type: {}", e.getClass().getName()); // 打印 IO 异常类型，区分 SSL、连接、超时等问题。
            log.error("[ToolChat] DeepSeek IO error message: {}", e.getMessage()); // 打印 IO 异常消息，方便定位网络或代理错误。
            if (e instanceof SSLHandshakeException) { // SSL 握手失败通常与代理、VPN、证书或 HTTP 版本有关。
                log.error("[ToolChat] DeepSeek SSL handshake failed. Check network/proxy/VPN/JDK HttpClient HTTP version/API endpoint."); // 按验收要求给出 SSL 排查方向。
            }
    ...
    private String realRagSearch(String query) { // Tool Calling 第二步：执行真实 RAG 检索。
        log.info("[ToolChat] real rag query: {}", query); // 打印真实 RAG 查询词。
        log.info("[ToolChat] vector store: Milvus"); // 明确当前向量库是 Milvus，不是 Redis。
        log.info("[ToolChat] milvus collection: {}", milvusCollectionName); // 打印 Milvus collection 名称，便于核对配置。
        List<String> contents = agentService.searchRagContents(query); // 复用已有 AgentService 中的 Milvus 向量检索逻辑。
        int resultCount = contents == null ? 0 : contents.size(); // 统计真实检索结果数量。
        String result = buildRagResult(contents); // 将知识片段拼成 tool result 文本。
        log.info("[ToolChat] real rag result count: {}", resultCount); // 打印真实 RAG 结果数量。
        log.info("[ToolChat] real rag result: {}", result); // 打印真实 RAG 结果内容，便于验收截图。
        return result; // 返回给第二次模型调用。
    }

    private String buildRagResult(List<String> contents) { // 将多个 RAG 片段拼接成工具返回文本。
        if (contents == null || contents.isEmpty()) { // 无命中时返回固定无结果文案。
            return "知识库中没有检索到与该问题相关的内容。";
        }
        StringBuilder builder = new StringBuilder("以下是从知识库检索到的相关内容：\n\n"); // tool result 头部说明内容来源。
        for (int i = 0; i < contents.size(); i++) { // 逐条拼接 Milvus 命中的知识片段。
            builder.append("片段").append(i + 1).append("：\n"); // 给每个片段加序号，便于模型区分上下文。
            builder.append(contents.get(i)).append("\n\n"); // 写入具体知识片段内容。
        }
        return builder.toString().trim(); // 去掉末尾空行后返回。
    }
