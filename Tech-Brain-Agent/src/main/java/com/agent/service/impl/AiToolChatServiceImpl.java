 ...
 *
 * <p>该类属于Tech-Brain-Agent业务模块，负责维持/api/ai/tool-chat现有业务流程：组装DeepSeek请求、解析tool_call、执行真实RAG检索、再把tool结果发回模型。</p>
 * <p>调用链为：AiToolChatController -> AiToolChatServiceImpl -> DeepSeekClient -> DeepSeek API；当模型返回ragSearch时，再调用AgentService.searchRagContents(query)检索Milvus。</p>
...
public class AiToolChatServiceImpl implements AiToolChatService { // 最小 Tool Calling 闭环实现。
    ...
    public AiToolChatServiceImpl(DeepSeekClient deepSeekClient, DeepSeekProperties deepSeekProperties) { // 构造器注入公共DeepSeek能力。
