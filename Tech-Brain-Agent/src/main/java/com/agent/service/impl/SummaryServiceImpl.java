...
public class SummaryServiceImpl implements SummaryService { // 通用内容总结服务实现类。
    ...
    public SummaryResult summarize(SummaryRequest request) {
        ...
        log.debug("[SummaryService] summarize content"); // 开始标记降级为DEBUG。
        log.debug("[SummaryService] normalized summaryType: {}", normalizedSummaryType); // 归一化细节降级为DEBUG。
        ...
        log.debug("[SummaryService] title: {}", previewContent(normalizedTitle)); // 标题只在DEBUG打印短preview。
        log.debug("[SummaryService] content length: {}", normalizedContent.length()); // 正文长度降级为DEBUG。
        log.debug("[SummaryService] content truncated: {}", truncated); // 截断状态降级为DEBUG。
        log.debug("[SummaryService] content preview: {}", previewContent(normalizedContent)); // 正文preview不能在INFO打印。
        ...
        log.debug("[SummaryService] summary preview: {}", previewContent(normalizedSummary)); // summary preview不能在INFO打印。
    ...
    private String normalizeSummaryType(String summaryType) {
        return SummaryTypeConstants.normalizeSummaryType(summaryType); // 复用通用 summaryType 归一化规则，非法值统一兜底 normal。
    }
    ...
    private String buildSummaryTypeInstruction(String summaryType) {
        return switch (summaryType) { // 按 summaryType 构造不同任务说明。
            case SummaryTypeConstants.POINTS -> "请使用项目符号或编号列表输出 5-8 个核心要点。\n" +
                    "每个要点尽量简洁，突出知识点、代码逻辑、关键步骤和注意事项。\n" +
                    "不要写成长篇散文，不要输出与 content 无关的扩展内容。"; // 要点总结。
            case SummaryTypeConstants.INTERVIEW -> "请严格按以下结构输出：\n" +
                    "1. 这篇内容主要讲什么\n" +
                    "2. 我在项目中如何理解\n" +
                    "3. 面试中可以这样表达\n" +
                    "4. 面试官可能追问\n" +
                    "5. 简化背诵版\n" +
                    "语言要适合 Java 后端 / AI 应用开发面试，可以结合项目表达，但不能编造 content 中没有的信息。\n" +
                    "简化背诵版要短，方便用户直接背诵。"; // 面试话术。
            default -> "请输出 3-5 段中文自然语言摘要。\n" +
                    "需要概括主题、核心内容和适用场景，保留关键技术点。\n" +
                    "如果涉及代码，只概括关键代码逻辑，不要输出过长代码。\n" +
                    "不要编造原文没有的信息。"; // 普通摘要。
        };
    }
