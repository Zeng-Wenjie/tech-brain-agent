 ...
 * 后台分页查询、详情查询、按工具名称统计、RAG 命中率统计和 CSV 导出能力。</p>
...
public interface ToolCallLogService extends IService<ToolCallLog> { // 继承 MyBatis-Plus 通用 Service 能力。
    ...
    ToolCallRagHitStatsVO statRagHit(ToolCallRagHitStatsRequest request); // 统计当前用户 ragSearch 工具的命中率和命中数量。

    List<ToolCallLogVO> listForExport(ToolCallLogExportRequest request); // 查询当前用户可导出的工具调用日志，最多返回 5000 条。
}
