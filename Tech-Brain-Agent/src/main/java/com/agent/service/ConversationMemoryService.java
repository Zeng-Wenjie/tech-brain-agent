 ...
 * -> ConversationMemoryServiceImpl -> DeepSeekClient 非流式摘要 -> ConversationMemoryMapper；ConversationServiceImpl 删除会话时也会通过本接口清理长期记忆。</p>
...
public interface ConversationMemoryService { // 会话长期记忆业务接口。
    ...
    void deleteByConversationAndUser(Long conversationId, Long userId); // 删除指定用户指定会话的长期记忆记录。
}
