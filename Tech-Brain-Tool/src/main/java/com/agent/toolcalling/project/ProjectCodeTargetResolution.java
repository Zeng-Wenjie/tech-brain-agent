 ...
 * analyzeCode 内部不同 analysisType 在路由阶段需要统一判断“用户这一轮到底指向什么目标”，
 * 本对象就是 {@link ProjectCodeTargetResolver} 的解析产物，描述目标类型、提取出的接口路径/类名/方法名，
 * 以及是否允许回退使用 recentProjectTarget 或 projectFileFocus。</p>
 *
 * <p>调用链：ToolCallingChatServiceImpl 路由阶段
 * -> ProjectCodeTargetResolver.resolve(userMessage, explicitProjectPath, hasRecentProjectTarget, hasProjectFocus, controllerFocus)
 * -> 返回本对象
...
public final class ProjectCodeTargetResolution { // 项目代码目标解析结果（不可变）。
    ...
    public enum TargetType { // 目标类型枚举。
        ...
        RECENT_PROJECT_TARGET, // 无明确目标，可优先使用 recentProjectTarget 的指代追问。
        FOCUS,    // 无明确目标且没有 recentProjectTarget 时，可使用 projectFileFocus 的指代追问。
