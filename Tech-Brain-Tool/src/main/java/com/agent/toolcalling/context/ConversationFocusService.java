 ...
 * 项目源码 projectFileFocus，后续工具路由可读取这些焦点解析“这篇笔记”“这个文件”“这个类”等指代。</p>
 * <p>当前调用链包括：RagSearchTool -> saveLastHitArticle -> Redis 文章焦点；
 * readFile 成功 -> saveActiveFileFocus -> Redis 上传文件焦点；
 * readProjectFile 成功 -> saveProjectFileFocus -> Redis 项目源码文件焦点。</p>
...
public class ConversationFocusService { // 保存会话最近命中文档焦点的通用服务。
    ...
    public void saveProjectFileFocus(Long userId,
        ...
        ConversationFocusContext context = buildProjectFileFocusContext(path, fileName, extension, language,
