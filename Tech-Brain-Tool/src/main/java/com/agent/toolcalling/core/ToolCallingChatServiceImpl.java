...
public class ToolCallingChatServiceImpl implements ToolCallingChatService { // 默认Tool Calling聊天编排实现。
    ...
    private static final String SYSTEM_PROMPT = "你是 Tech-Brain 项目的 AI 助手。" + // 边界优化版system prompt，定义助手身份。
            ...
            "\n- 如果用户要搜索代码，应等待后续 SearchCodeTool；如果用户要读取指定项目代码文件，应等待后续 readProjectFile。" +
    ...
    public void chatStream(String currentMessage,
        ...
        try { // 统一兜底Tool Calling流式编排中的异常。
            ...
            boolean forceListProjectTree = shouldForceListProjectTree(userMessage); // 用户要求查看项目目录结构时强制执行listProjectTree。
            ...
            String answerMode = forceListProjectTree ? "force-listProjectTree"
    ...
    private void streamWithForcedListProjectTree(String userMessage,
        ...
        ObjectNode streamRequest = buildForcedProjectTreeAnswerRequest(userMessage, memorySummary, historyMessages,
                toolResult, requestContext); // 将目录树结果注入最终流式回答。
        log.debug("[ToolChatStream] before forced listProjectTree DeepSeek stream call"); // DeepSeek流式调用细节降级为DEBUG。
        deepSeekClient.streamChatCompletions(streamRequest, new DeepSeekStreamCallback() { // 基于目录树结果流式生成最终回答。
            @Override
            public void onToken(String token) { // DeepSeek返回增量token。
                callback.onToken(token); // 转发给/chat/message的SSE回调。
            }
            ...
            @Override
            public void onError(Throwable error) { // 强制listProjectTree后的流式模型调用异常。
        ...
        log.debug("[ToolChatStream] after forced listProjectTree DeepSeek stream call"); // DeepSeek流式调用细节降级为DEBUG。
    }
