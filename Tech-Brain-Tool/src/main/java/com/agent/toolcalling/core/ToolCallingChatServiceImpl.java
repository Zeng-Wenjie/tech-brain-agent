...
public class ToolCallingChatServiceImpl implements ToolCallingChatService { // 默认Tool Calling聊天编排实现。
    ...
    private static final String SYSTEM_PROMPT = "你是 Tech-Brain 项目的 AI 助手。" + // 边界优化版system prompt，定义助手身份。
            ...
            "\n6. analyzeCode：用于分析项目 workspace 内单个源码文件结构，提取类、字段、方法、Spring 接口和 AI Tool 基础信息，不返回完整源码。" +
            "\n7. analyzeCallChain：用于分析项目 workspace 内单个源码文件的轻量调用链，提取当前类依赖、方法内部调用、对象调用和候选目标文件，不做完整调用图，不返回完整源码。" +
            "\n8. analyzeControllerServiceChain：用于分析 Spring Controller 到 Service 的接口调用链，提取接口路径、Controller 方法、Service 调用和 Service/ServiceImpl 候选文件，不做完整调用图，不返回完整源码。" +
            "\n9. analyzeToolServiceChain：用于分析 AI Tool 实现类到业务 Service、Guard、Registry、Mapper、Repository 等组件的专项调用链，提取 toolName、execute 入口、依赖组件和候选目标文件，不做完整调用图，不返回完整源码。" +
            "\n10. analyzeSseEventChain：用于分析前后端 SSE 事件链路，定位前端 SSE 请求发起点、前端事件接收点、后端 Controller SSE 接口、Controller→Service 调用和后端 SSE 事件发送点，跨前后端全项目扫描，不返回完整源码。" +
            "\n11. readProjectFile：用于读取项目 workspace 内指定代码/文本文件内容，必须传相对 workspace 的文件路径，禁止读取敏感文件和 workspace 外路径。" +
            "\n\n调用 ragSearch 的规则：" +
            ...
            "\n- 如果用户要搜索代码位置，应调用 searchCode；如果用户要分析指定项目源码文件结构，应调用 analyzeCode；如果用户要读取指定项目代码文件，应调用 readProjectFile。" +
            ...
            "\n\n调用 analyzeCode 的规则：" +
            "\n- 当用户要求分析某个项目源码文件/类/工具的代码结构、类结构、文件结构、方法列表、字段列表、注解、Controller 接口或 Tool 工具名时，应调用 analyzeCode。" +
            "\n- analyzeCode 必须传相对 workspace 的 path；如果只有类名或文件名，应先确保能唯一定位到文件；如果当前会话有 projectFileFocus，可用于“这个类/这个工具/这个文件有哪些方法”等追问。" +
            "\n- analyzeCode 只返回结构化分析结果，不返回完整源码，不做 AST、跨文件调用链、风险分析、测试步骤生成或结果持久化，不修改项目代码。" +
            "\n\n调用 analyzeCallChain 的规则：" +
            "\n- 当用户要求分析某个项目源码文件/类/工具/方法的调用关系、调用链、方法调用、依赖了哪些 Service/Mapper/Repository/Tool、内部调用、外部调用、这个方法调用了什么、这个类依赖哪些时，应调用 analyzeCallChain。" +
            "\n- analyzeCallChain 必须传相对 workspace 的 path；如果只指定方法名，可传 methodName 限定入口方法；如果当前会话有 projectFileFocus，可用于“分析这个文件的调用链/这个类调用了哪些”等追问。" +
            "\n- analyzeCallChain 只做轻量级静态调用关系候选提取，候选目标文件不保证完全精准；不做完整调用图、跨仓库调用链、风险分析、测试步骤生成或结果持久化，不返回完整源码，不修改项目代码。" +
            "\n- analyzeCallChain 用于调用链/调用关系分析；analyzeCode 用于单文件结构分析；readProjectFile 用于读取完整代码；searchCode 用于定位文件和代码片段；ragSearch 不用于分析项目 workspace 源码。" +
            "\n\n调用 analyzeControllerServiceChain 的规则：" +
            "\n- 当用户要求分析 Spring Controller 接口到 Service 的调用链、接口背后调用了哪些 Service、某个 /api 接口后面调用了哪些方法、Controller 的某个接口链路时，应调用 analyzeControllerServiceChain。" +
            "\n- analyzeControllerServiceChain 可传相对 workspace 的 Controller 文件 path，也可只传 endpoint（接口路径）；当用户输入 /api/agent/chat、/chat/message 这类 API 路径时必须放入 endpoint，不能放入 path；可选 methodName（Controller 方法名）；maxDepth=2 时会尝试分析 ServiceImpl 内部一层调用；如果当前 projectFileFocus 是 Controller 文件，可用于“分析接口调用链/这个接口调用了哪些 Service”等追问。" +
            "\n- analyzeControllerServiceChain 用于 Controller→Service 专项链路分析；analyzeCallChain 用于普通类轻量调用链；analyzeCode 用于单文件结构分析；readProjectFile 用于读取完整代码；searchCode 用于定位文件；ragSearch 不用于分析项目 workspace 源码。" +
            "\n- analyzeControllerServiceChain 只做静态文本级分析，候选 ServiceImpl 不保证完全精准；不做完整调用图、不做风险分析、不做测试步骤、不返回完整源码、不修改项目代码。" +
            "\n\n调用 analyzeToolServiceChain 的规则：" +
            "\n- 当用户要求分析 AI Tool 到业务 Service、Guard、Registry、Mapper、Repository 的调用链，或询问某个工具背后调用了哪些组件、哪些业务类、execute 调用了什么时，应调用 analyzeToolServiceChain。" +
            "\n- analyzeToolServiceChain 可传相对 workspace 的 Tool 文件 path，也可传 toolName（如 searchCode、readProjectFile、analyzeCode）或 className（如 SearchCodeTool）；path、toolName、className 至少一个。" +
            "\n- analyzeToolServiceChain 用于 Tool→Service 专项链路分析；analyzeControllerServiceChain 用于 Controller→Service 专项链路分析；analyzeCallChain 用于普通类轻量调用链；analyzeCode 用于单文件结构分析；readProjectFile 用于读取完整代码；searchCode 用于定位文件和代码片段；listProjectTree 用于查看目录结构；ragSearch 不用于分析项目 workspace 源码。" +
            "\n- analyzeToolServiceChain 只做静态文本级分析，候选目标文件不保证完全精准；不做完整调用图、不做风险分析、不做测试步骤、不返回完整源码、不修改项目代码。" +
            "\n\n调用 analyzeSseEventChain 的规则：" +
            "\n- 当用户要求分析前后端 SSE、流式响应、流式输出、事件流、事件链路、前端到后端流式通信，或问“这个接口怎么流式返回”“前端哪里调用/接收这个接口”“某个事件后端哪里发送/前端哪里接收”时，应调用 analyzeSseEventChain。" +
            "\n- analyzeSseEventChain 可传 endpoint（如 /chat/message）、eventName（如 summary_result、message、done、error）、frontendKeyword（如 EventSource、fetch、onmessage、addEventListener、TextDecoder）、methodName 或 path，至少一个；endpoint/eventName 明确时全项目扫描前后端，不被 projectFileFocus 限制。" +
            "\n- analyzeSseEventChain 用于前端触发点 → 后端 SSE Controller → Service → Tool/业务逻辑 → SSE 事件发送 → 前端事件接收 的链路分析；analyzeControllerServiceChain 只用于后端 Controller→Service；analyzeToolServiceChain 只用于 AI Tool→Service/组件；analyzeCallChain 用于普通类调用链；analyzeCode 用于单文件结构分析；readProjectFile 用于读取完整代码；searchCode 用于定位文件和关键词片段；listProjectTree 用于查看目录结构；ragSearch 不用于分析项目 workspace 源码。" +
            "\n- analyzeSseEventChain 只做静态文本级分析，跨前后端链路为候选结果不保证完全精准；不做完整调用图、不做风险分析、不做测试步骤、不返回完整源码、不修改前端或后端业务代码。" +
    ...
    private static final String ANALYZE_CALL_CHAIN_TOOL_NAME = "analyzeCallChain"; // 项目代码调用链分析工具名。

    private static final String ANALYZE_CONTROLLER_SERVICE_CHAIN_TOOL_NAME = "analyzeControllerServiceChain"; // Controller→Service 调用链分析工具名。

    private static final String ANALYZE_TOOL_SERVICE_CHAIN_TOOL_NAME = "analyzeToolServiceChain"; // AI Tool→业务组件调用链分析工具名。

    private static final String ANALYZE_SSE_EVENT_CHAIN_TOOL_NAME = "analyzeSseEventChain"; // 前后端 SSE 事件链路分析工具名。
    ...
    private static final String ROUTE_REASON_FORCE_ANALYZE_CALL_CHAIN = "force analyzeCallChain"; // 强制执行 analyzeCallChain 的路由原因。

    private static final String ROUTE_REASON_FORCE_ANALYZE_CALL_CHAIN_BY_FOCUS = "force analyzeCallChain by projectFileFocus"; // 通过项目文件焦点强制执行 analyzeCallChain 的路由原因。

    private static final String ROUTE_REASON_FORCE_ANALYZE_CONTROLLER_SERVICE_CHAIN = "force analyzeControllerServiceChain"; // 强制执行 analyzeControllerServiceChain 的路由原因。

    private static final String ROUTE_REASON_FORCE_ANALYZE_CONTROLLER_SERVICE_CHAIN_BY_ENDPOINT = "force analyzeControllerServiceChain by endpoint"; // 通过接口路径强制执行 analyzeControllerServiceChain 的路由原因。

    private static final String ROUTE_REASON_FORCE_ANALYZE_CONTROLLER_SERVICE_CHAIN_BY_FOCUS = "force analyzeControllerServiceChain by projectFileFocus"; // 通过 Controller 文件焦点强制执行 analyzeControllerServiceChain 的路由原因。

    private static final String ROUTE_REASON_FORCE_ANALYZE_TOOL_SERVICE_CHAIN = "force analyzeToolServiceChain"; // 强制执行 analyzeToolServiceChain 的路由原因。

    private static final String ROUTE_REASON_FORCE_ANALYZE_TOOL_SERVICE_CHAIN_BY_TOOL_NAME = "force analyzeToolServiceChain by toolName"; // 通过 toolName 强制执行 analyzeToolServiceChain 的路由原因。

    private static final String ROUTE_REASON_FORCE_ANALYZE_TOOL_SERVICE_CHAIN_BY_FOCUS = "force analyzeToolServiceChain by projectFileFocus"; // 通过 AI Tool 文件焦点强制执行 analyzeToolServiceChain 的路由原因。
    ...
    private static final String ROUTE_REASON_FORCE_ANALYZE_SSE_EVENT_CHAIN = "force analyzeSseEventChain"; // 强制执行 analyzeSseEventChain 的路由原因。
    ...
    private static final String ANALYZE_CODE_FINAL_ANSWER_SYSTEM_PROMPT = "当前工具是 analyzeCode。" // analyzeCode 专项最终回答规则。
            + "\n必须基于 result_json 中的 code_analysis 结果回答，不要编造未返回的类、字段、方法、接口或工具信息。"
            + "\n如果 success=false，只说明工具返回的失败原因。"
            + "\n如果 success=true，按用户问题总结文件概览、classInfo、fields、methods、springEndpoints 和 aiToolInfo；没有的部分可以简要说明未识别到。"
            + "\n如果 result_json.truncated=true，需要说明结构项已按 maxItems 截断。"
            + "\n不要输出完整源码，不要输出完整 result_json，不要生成 patch，不要声称修改了文件。";

    private static final String ANALYZE_CALL_CHAIN_FINAL_ANSWER_SYSTEM_PROMPT = "当前工具是 analyzeCallChain。" // analyzeCallChain 专项最终回答规则。
            + "\n必须基于 result_json 中的 call_chain_analysis 结果回答，不要编造未返回的依赖、方法调用、对象调用或候选文件。"
            + "\n如果 success=false，只说明工具返回的失败原因。"
            + "\n如果 success=true，按以下顺序组织回答：文件概览、入口方法（如果有 entryMethod）、依赖对象 dependencies、内部方法调用 internalCalls、外部对象调用 externalCalls、候选目标文件 candidateTargets、不确定项（utilityCalls/unresolvedCalls）说明、简短调用链总结。"
            + "\n候选目标文件 candidateTargets 只是静态文本推断的候选，必须说明不保证完全精准。"
            + "\n如果 result_json.truncated=true，需要说明调用项已按 maxItems 截断。"
            + "\n不要输出完整源码，不要输出完整 result_json，不要生成风险分析、测试步骤或修改方案，不要生成 patch，不要声称修改了文件。";

    private static final String ANALYZE_CONTROLLER_SERVICE_CHAIN_FINAL_ANSWER_SYSTEM_PROMPT = "当前工具是 analyzeControllerServiceChain。" // analyzeControllerServiceChain 专项最终回答规则。
            + "\n必须基于 result_json 中的 controller_service_chain_analysis 结果回答，不要编造未返回的接口、Service 调用或候选文件。"
            + "\n如果 arguments_json 只有 endpoint 没有 path，应说明这是按接口路径在 Controller 中匹配方法后再分析 Controller→Service 调用链，不要把 endpoint 说成源码文件路径。"
            + "\n如果 success=false，只说明工具返回的失败原因。"
            + "\n如果失败结果包含 candidateEndpoints，应提示“未匹配到该 endpoint，请确认接口完整路径”，并列出相似接口，不要说“该路径不是后端源码文件”。"
            + "\n如果 result_json.matchedBy=ENDPOINT_GLOBAL_SCAN，说明这是按接口路径在全项目所有 Controller 中扫描定位出来的：成功时先说明“基于全项目 Controller 扫描，找到 {endpoint} 对应的 Controller：{matchedControllerClass}（{matchedControllerPath}）”，再展示链路；不要说“当前 Controller 中没有/无法分析”。"
            + "\n如果 success=true，按以下顺序组织回答：Controller 概览（className/basePath）、接口列表 endpoints、Controller 方法 → Service 方法映射 serviceCalls、Service 接口/ServiceImpl 候选路径 candidateServiceFiles、如果有 serviceMethodCalls 则展示 ServiceImpl 内部一层调用、不确定项说明、简短链路总结。"
            + "\nendpoints 的 path 已是 basePath 与方法级路径拼接结果；候选 ServiceImpl 文件只是静态文本推断，必须说明不保证完全精准。"
            + "\n如果 success=false 且 matchedBy=ENDPOINT_GLOBAL_SCAN，说明“已扫描项目内所有 Controller（含所有子模块），未找到该接口”，并列出 candidateEndpoints 中的相似接口候选；不要说“当前 Controller 中没有，所以无法分析”。"
            + "\n绝对禁止根据接口路径名编造 Controller 类名或文件（例如把 /batch 脑补成 BatchArticleController、把 /xxx 拼成 XxxController）；只能引用 result_json 中真实扫描到的 matchedControllerClass、matchedControllerPath 和 candidateEndpoints，没有就如实说未找到。"
            + "\n如果 result_json.truncated=true，需要说明接口/调用项已按 maxItems 截断。"
            + "\n不要输出完整源码，不要输出完整 result_json，不要生成风险分析、测试步骤或修改方案，不要生成 patch，不要声称修改了文件。";

    private static final String ANALYZE_TOOL_SERVICE_CHAIN_FINAL_ANSWER_SYSTEM_PROMPT = "当前工具是 analyzeToolServiceChain。" // analyzeToolServiceChain 专项最终回答规则。
            + "\n必须基于 result_json 中的 tool_service_chain_analysis 结果回答，不要编造未返回的 Tool、依赖、调用或候选文件。"
            + "\n如果 success=false，只说明工具返回的失败原因；如果包含 candidateTools，应列出候选 Tool，让用户指定 path 或 className。"
            + "\n如果 success=true，按以下顺序组织回答：Tool 概览、toolName/className、入口方法 execute、依赖组件 dependencies、内部方法调用 internalCalls、Service/Guard/Registry/Mapper/Repository 调用、候选目标文件 candidateTargets、如果有 serviceMethodCalls 则展示 ServiceImpl 内部一层调用、不确定项说明、简短链路总结。"
            + "\n候选目标文件 candidateTargets 只是静态文本推断的候选，必须说明不保证完全精准。"
            + "\n如果 result_json.truncated=true，需要说明调用项已按 maxItems 截断。"
            + "\n不要输出完整源码，不要输出完整 result_json，不要生成风险分析、测试步骤或修改方案，不要生成 patch，不要声称修改了文件。";

    private static final String ANALYZE_SSE_EVENT_CHAIN_FINAL_ANSWER_SYSTEM_PROMPT = "当前工具是 analyzeSseEventChain。" // analyzeSseEventChain 专项最终回答规则。
            + "\n必须基于 result_json 中的 sse_event_chain_analysis 结果回答，不要编造未返回的事件、接口、类、文件或方法。"
            + "\n如果 success=false，只说明工具返回的失败原因，并列出 candidateEndpoints / candidateEvents 中的真实候选。"
            + "\n如果 success=true，按以下顺序组织回答：链路概览、前端请求发起点 frontendCalls、前端事件接收/消费点 frontendEventHandlers、后端 SSE Controller backendSseEndpoints、Controller→Service 调用 serviceCalls、Service/Tool 内部候选链路 toolCalls、后端事件发送点 backendEventSenders、前后端事件名对应关系、不确定项说明，最后给出一个简短链路图（前端→POST endpoint→Controller.method→Service.method→事件发送→前端处理）。"
            + "\n如果 frontendCalls / frontendEventHandlers 为空，应如实说明 workspace 内未扫描到前端 SSE 代码，不要编造前端文件。"
            + "\n候选 ServiceImpl/Tool 文件只是静态文本推断，必须说明不保证完全精准。"
            + "\n如果 result_json.truncated=true，需要说明结果已按 maxItems 截断。"
            + "\n不要输出完整源码，不要输出完整 result_json，不要生成风险分析、测试步骤或修改方案，不要生成 patch，不要声称修改了文件。";
    ...
    private static final String TOOL_RESULT_SYSTEM_PROMPT = "你是 Tech-Brain 项目的 AI 助手。" + // 模型主动tool_call后的最终回答约束。
            ...
            "\n如果工具结果是 code_analysis 类型：只能基于返回的 classInfo、fields、methods、springEndpoints、aiToolInfo 和 basicSymbols 说明代码结构；不要输出完整源码，不要生成 patch。" +
    ...
    private static final String[] PROJECT_TOOL_KNOWN_NAMES = { // 当前项目已知 AI Tool 名称，用于工具追问和存在性判断。
            ...
            "analyzeCallChain",
            "analyzeControllerServiceChain",
            "analyzeToolServiceChain",
            "analyzeSseEventChain",
            "readProjectFile",
            "readFile",
            "ragSearch",
            "summarizeArticle"
    ...
    private static final String[] TOOL_SERVICE_CHAIN_ORDINARY_CALL_CHAIN_KEYWORDS = { // 用户明确说“普通调用链”时保留给 analyzeCallChain，避免 Tool 专项误抢。
            "普通调用链",
            "普通调用关系",
            "一般调用链"
    };
    ...
    public void chatStream(String currentMessage,
        ...
        try { // 统一兜底Tool Calling流式编排中的异常。
            ...
            ProjectCodeTargetResolution projectCodeTarget = projectCodeTargetResolver.resolve(userMessage,
                    ...
                    isControllerFocus(requestContext)); // 统一目标解析：明确目标 > 全项目扫描 > projectFileFocus。
            ...
            boolean forceSseEventChain = !forceSummarizeArticle && !forceReadProjectFile && shouldForceAnalyzeSseEventChain(userMessage, requestContext); // 前后端 SSE 事件链路优先于 Controller→Service 等链路。
            log.info("[ToolChatStream] force analyzeSseEventChain: {}", forceSseEventChain); // 打印强制 SSE 事件链路路由判断结果。
            boolean forceControllerServiceChain = !forceSummarizeArticle && !forceReadProjectFile && !forceSseEventChain && shouldForceAnalyzeControllerServiceChain(userMessage, requestContext); // Controller→Service 接口链路优先于普通调用链。
            log.info("[ToolChatStream] force analyzeControllerServiceChain: {}", forceControllerServiceChain); // 打印强制 Controller→Service 链路路由判断结果。
            boolean forceToolServiceChain = !forceSummarizeArticle && !forceReadProjectFile && !forceSseEventChain && !forceControllerServiceChain && shouldForceAnalyzeToolServiceChain(userMessage, requestContext); // Tool→业务组件链路优先于普通调用链。
            log.info("[ToolChatStream] force analyzeToolServiceChain: {}", forceToolServiceChain); // 打印强制 Tool→业务组件链路路由判断结果。
            boolean forceAnalyzeCallChain = !forceSummarizeArticle && !forceReadProjectFile && !forceSseEventChain && !forceControllerServiceChain && !forceToolServiceChain && shouldForceAnalyzeCallChain(userMessage, requestContext); // 调用链/调用关系/依赖分析优先于单文件结构分析。
            log.info("[ToolChatStream] force analyzeCallChain: {}", forceAnalyzeCallChain); // 打印强制调用链分析路由判断结果。
            boolean forceAnalyzeCode = !forceSummarizeArticle && !forceReadProjectFile && !forceSseEventChain && !forceControllerServiceChain && !forceToolServiceChain && !forceAnalyzeCallChain && shouldForceAnalyzeCode(userMessage, requestContext); // 结构/方法/字段/接口分析优先于项目文件焦点读取。
            log.info("[ToolChatStream] force analyzeCode: {}", forceAnalyzeCode); // 打印强制代码结构分析路由判断结果。
            boolean forceProjectFileFocus = !forceSummarizeArticle && !forceReadProjectFile && !forceSseEventChain && !forceControllerServiceChain && !forceToolServiceChain && !forceAnalyzeCallChain && !forceAnalyzeCode && shouldUseProjectFileFocus(userMessage, requestContext); // 项目源码文件追问优先使用最近 readProjectFile 焦点。
            ...
            boolean forceReadFile = !forceReadProjectFile && !forceSseEventChain && !forceControllerServiceChain && !forceToolServiceChain && !forceAnalyzeCallChain && !forceAnalyzeCode && !forceProjectFileFocus && !promptProjectFileFocusMissing && shouldForceReadFile(userMessage, requestContext); // 明确要求读取/分析当前附件时由后端强制readFile，避免模型漏调工具。
            log.info("[ToolChatStream] force readFile: {}", forceReadFile); // 打印强制readFile路由判断结果。
            boolean forceListProjectTree = !forceReadProjectFile && !forceSseEventChain && !forceControllerServiceChain && !forceToolServiceChain && !forceAnalyzeCallChain && !forceAnalyzeCode && !forceProjectFileFocus && !promptProjectFileFocusMissing && !forceReadFile && shouldForceListProjectTree(userMessage); // 用户要求查看项目目录结构时强制执行listProjectTree。
            log.info("[ToolChatStream] force listProjectTree: {}", forceListProjectTree); // 打印强制目录树路由判断结果。
            boolean forceSearchCode = !forceReadProjectFile && !forceSseEventChain && !forceControllerServiceChain && !forceToolServiceChain && !forceAnalyzeCallChain && !forceAnalyzeCode && !forceProjectFileFocus && !promptProjectFileFocusMissing && !forceReadFile && !forceListProjectTree && shouldForceSearchCode(userMessage); // 用户要求定位类、方法、注解或关键词位置时强制执行searchCode。
            ...
            if (forceSummarizeArticle) { // 总结文章强制路由。
                ...
                answerMode = "force-analyzeSseEventChain"; // 标记 SSE 事件链路分析模式。
            } else if (forceControllerServiceChain) { // Controller→Service 接口链路分析。
                answerMode = "force-analyzeControllerServiceChain"; // 标记 Controller→Service 链路分析模式。
            } else if (forceToolServiceChain) { // AI Tool→业务组件链路分析。
                answerMode = "force-analyzeToolServiceChain"; // 标记 Tool→业务组件链路分析模式。
            } else if (forceAnalyzeCallChain) { // 项目文件调用链分析。
                answerMode = "force-analyzeCallChain"; // 标记调用链分析模式。
            ...
            if (forceSseEventChain) { // 用户明确要求分析前后端 SSE 事件链路时，跳过 Controller→Service 等链路和模型tool_call判断。
                streamWithForcedSseEventChain(userMessage, normalizedMemorySummary, normalizedHistoryMessages, requestContext, callback, streamErrorNotified); // 直接执行analyzeSseEventChain并进入流式回答。
            ...
            if (forceControllerServiceChain) { // 用户明确要求分析 Controller→Service 接口链路时，跳过普通调用链和模型tool_call判断。
                streamWithForcedControllerServiceChain(userMessage, normalizedMemorySummary, normalizedHistoryMessages, requestContext, callback, streamErrorNotified); // 直接执行analyzeControllerServiceChain并进入流式回答。
            ...
            if (forceToolServiceChain) { // 用户明确要求分析 AI Tool→业务组件链路时，跳过普通调用链和模型tool_call判断。
                streamWithForcedToolServiceChain(userMessage, normalizedMemorySummary, normalizedHistoryMessages, requestContext, callback, streamErrorNotified); // 直接执行analyzeToolServiceChain并进入流式回答。
            ...
            if (forceAnalyzeCallChain) { // 用户明确要求分析项目源码调用链时，跳过analyzeCode结构分析和模型tool_call判断。
                streamWithForcedAnalyzeCallChain(userMessage, normalizedMemorySummary, normalizedHistoryMessages, requestContext, callback, streamErrorNotified); // 直接执行analyzeCallChain并进入流式回答。
                return; // 强制analyzeCallChain路径结束后不再走结构分析、目录树、搜索、RAG、readFile或模型tool_call链路。
            }
            if (forceAnalyzeCode) { // 用户明确要求分析项目源码结构时，跳过readProjectFile全文输出和模型tool_call判断。
                streamWithForcedAnalyzeCode(userMessage, normalizedMemorySummary, normalizedHistoryMessages, requestContext, callback, streamErrorNotified); // 直接执行analyzeCode并进入流式回答。
            ...
            if (ANALYZE_CALL_CHAIN_TOOL_NAME.equals(toolName)) { // 模型主动调用 analyzeCallChain 成功后也要更新项目文件焦点。
                saveProjectFileFocusFromCallChainResult(toolResult, requestContext); // 保存相对路径和元信息，不保存文件内容。
            }
            if (ANALYZE_CONTROLLER_SERVICE_CHAIN_TOOL_NAME.equals(toolName)) { // 模型主动调用 analyzeControllerServiceChain 成功后也要更新项目文件焦点。
                saveProjectFileFocusFromControllerServiceChainResult(toolResult, requestContext); // 保存相对路径和元信息，不保存文件内容。
            }
            if (ANALYZE_TOOL_SERVICE_CHAIN_TOOL_NAME.equals(toolName)) { // 模型主动调用 analyzeToolServiceChain 成功后也要更新项目文件焦点。
                saveProjectFileFocusFromToolServiceChainResult(toolResult, requestContext); // 保存相对路径和元信息，不保存文件内容。
            }
    ...
    private void streamWithForcedAnalyzeCode(String userMessage,
                                             ...
                                             ToolCallingRequestContext requestContext,
                                             ToolCallingStreamCallback callback,
                                             boolean[] streamErrorNotified) { // 流式接口的后端强制analyzeCode闭环。
        log.info("[ToolChatStream] force execute tool: {}", ANALYZE_CODE_TOOL_NAME); // 打印强制执行的工具名。
        log.info("[ToolChatStream] rag search status: {}", RagSearchStatus.NOT_SEARCHED); // 代码结构分析分支没有执行ragSearch。
        ...
        AnalyzeCodeTarget target = resolveAnalyzeCodeTargetFromMessage(userMessage, requestContext); // 解析明确路径、类名/文件名或projectFileFocus。
        if (target == null || target.path() == null || target.path().isBlank()) { // 没有明确文件目标时不乱猜。
            callback.onToken(buildProjectFileFocusMissingPrompt()); // 提示用户指定类名、路径或先搜索。
            callback.onComplete(); // 结束本轮SSE。
            return; // 不执行工具。
        }

        log.info("[ToolChatStream] force analyzeCode path: {}, byFocus: {}",
                target.path(), target.byFocus()); // 只打印workspace相对路径。
        ObjectNode arguments = objectMapper.createObjectNode(); // 构造analyzeCode工具入参。
        arguments.put("path", target.path()); // path 必填，工具内部会再次校验。
        arguments.put("includeSnippet", true); // 默认带声明行片段，便于最终回答。
        arguments.put("maxItems", FORCE_ANALYZE_CODE_MAX_ITEMS); // 强制路由使用默认结构项上限。
        String routeReason = target.byFocus() ? ROUTE_REASON_FORCE_ANALYZE_CODE_BY_FOCUS : ROUTE_REASON_FORCE_ANALYZE_CODE; // 区分显式目标和焦点目标。
        ...
        logToolResult(toolResult); // 工具结果只打preview，不打印完整结构JSON。
    ...
    private void streamWithForcedAnalyzeCallChain(String userMessage,
                                                  String memorySummary,
                                                  List<ToolChatHistoryMessage> historyMessages,
                                                  ToolCallingRequestContext requestContext,
                                                  ToolCallingStreamCallback callback,
                                                  boolean[] streamErrorNotified) { // 流式接口的后端强制analyzeCallChain闭环。
        log.info("[ToolChatStream] force execute tool: {}", ANALYZE_CALL_CHAIN_TOOL_NAME); // 打印强制执行的工具名。
        log.info("[ToolChatStream] rag search status: {}", RagSearchStatus.NOT_SEARCHED); // 调用链分析分支没有执行ragSearch。
        AiTool analyzeCallChainTool = toolRegistry.getTool(ANALYZE_CALL_CHAIN_TOOL_NAME); // 从ToolRegistry获取AnalyzeCallChainTool。
        if (analyzeCallChainTool == null) { // analyzeCallChain未注册时不能伪造分析结果。
            callback.onToken("系统错误：analyzeCallChain 工具未注册。"); // 通过SSE返回明确错误。
            callback.onComplete(); // 结束本轮SSE。
            return; // 工具缺失时不再继续。
        }

        AnalyzeCodeTarget target = resolveAnalyzeCodeTargetFromMessage(userMessage, requestContext); // 复用 analyzeCode 目标解析：明确路径、类名/文件名或projectFileFocus。
        if (target == null || target.path() == null || target.path().isBlank()) { // 没有明确文件目标时不乱猜。
            callback.onToken(buildProjectFileFocusMissingPrompt()); // 提示用户指定类名、路径或先搜索。
            callback.onComplete(); // 结束本轮SSE。
            return; // 不执行工具。
        ...
        String methodName = extractCallChainMethodName(userMessage); // 从用户输入中提取可选入口方法名。
        log.info("[ToolChatStream] force analyzeCallChain path: {}, byFocus: {}, methodName: {}",
                target.path(), target.byFocus(), methodName); // 只打印workspace相对路径和方法名。
        ObjectNode arguments = objectMapper.createObjectNode(); // 构造analyzeCallChain工具入参。
        arguments.put("path", target.path()); // path 必填，工具内部会再次校验。
        if (methodName != null && !methodName.isBlank()) { // 指定入口方法时才传 methodName。
            arguments.put("methodName", methodName); // 限定分析范围到该方法。
        }
        arguments.put("maxDepth", FORCE_ANALYZE_CALL_CHAIN_MAX_DEPTH); // 第一版强制路由固定深度 1。
        arguments.put("maxItems", FORCE_ANALYZE_CALL_CHAIN_MAX_ITEMS); // 强制路由使用默认调用项上限。
        arguments.put("includeSnippet", true); // 默认带调用表达式片段，便于最终回答。
        String routeReason = target.byFocus() ? ROUTE_REASON_FORCE_ANALYZE_CALL_CHAIN_BY_FOCUS : ROUTE_REASON_FORCE_ANALYZE_CALL_CHAIN; // 区分显式目标和焦点目标。
        String toolResult = executeToolWithLog(analyzeCallChainTool, arguments, requestContext,
                CALL_SOURCE_FORCE_ROUTE, routeReason); // 执行analyzeCallChain并记录tool_call_log。
        logToolResult(toolResult); // 工具结果只打preview，不打印完整调用链JSON。
        saveProjectFileFocusFromCallChainResult(toolResult, requestContext); // 成功分析后刷新projectFileFocus元信息。

        String argumentsJson = serializeArguments(arguments); // 复用日志入参序列化，保证最终回答阶段明确当前path。
        ObjectNode streamRequest = buildToolFinalAnswerRequest(userMessage, ANALYZE_CALL_CHAIN_TOOL_NAME, argumentsJson, toolResult); // analyzeCallChain最终回答只保留本轮工具结果。
        streamSafeToolFinalAnswer(ANALYZE_CALL_CHAIN_TOOL_NAME, userMessage, argumentsJson, toolResult,
                streamRequest, callback, streamErrorNotified); // 统一检测并拦截DSML/tool_calls泄漏。
    }

    private void streamWithForcedControllerServiceChain(String userMessage,
                                                        String memorySummary,
                                                        List<ToolChatHistoryMessage> historyMessages,
                                                        ToolCallingRequestContext requestContext,
                                                        ToolCallingStreamCallback callback,
                                                        boolean[] streamErrorNotified) { // 流式接口的后端强制 analyzeControllerServiceChain 闭环。
        log.info("[ToolChatStream] force execute tool: {}", ANALYZE_CONTROLLER_SERVICE_CHAIN_TOOL_NAME); // 打印强制执行的工具名。
        log.info("[ToolChatStream] rag search status: {}", RagSearchStatus.NOT_SEARCHED); // Controller→Service 链路分支没有执行ragSearch。
        AiTool controllerServiceChainTool = toolRegistry.getTool(ANALYZE_CONTROLLER_SERVICE_CHAIN_TOOL_NAME); // 从ToolRegistry获取工具。
        if (controllerServiceChainTool == null) { // 工具未注册时不能伪造分析结果。
            callback.onToken("系统错误：analyzeControllerServiceChain 工具未注册。"); // 通过SSE返回明确错误。
            callback.onComplete(); // 结束本轮SSE。
            return; // 工具缺失时不再继续。
        }

        String endpoint = extractControllerEndpoint(userMessage); // 先提取接口路径，/api/xxx 这类 endpoint 不能被当作项目文件 path。
        boolean endpointQuery = endpoint != null && !endpoint.isBlank(); // 是否为 endpoint 查询。
        // endpoint 查询优先级最高：只传 endpoint 让工具全项目扫描 Controller，绝不绑定 projectFileFocus 或上一轮分析过的 Controller。
        AnalyzeCodeTarget target = endpointQuery
                ? null
                : resolveControllerTargetFromMessage(userMessage, requestContext); // 仅在没有 endpoint 时才解析显式 Controller 目标或 Controller 焦点。
        if (!endpointQuery
                && (target == null || target.path() == null || target.path().isBlank())) { // 没有 endpoint 也没有明确 Controller 文件目标时不乱猜。
            callback.onToken(buildProjectFileFocusMissingPrompt()); // 提示用户指定 Controller 类名、路径或先搜索。
            callback.onComplete(); // 结束本轮SSE。
            return; // 不执行工具。
        }

        String methodName = extractControllerMethodName(userMessage); // 从用户输入中提取可选 Controller 方法名。
        int maxDepth = isServiceImplDepthIntent(userMessage) ? 2 : 1; // “ServiceImpl/实现类/内部”等意图时使用 maxDepth=2。
        log.info("[ToolChatStream] force analyzeControllerServiceChain path: {}, byFocus: {}, endpoint: {}, methodName: {}, maxDepth: {}",
                target == null ? null : target.path(), target != null && target.byFocus(), endpoint, methodName, maxDepth); // 只打印workspace相对路径与过滤条件。
        ObjectNode arguments = objectMapper.createObjectNode(); // 构造工具入参。
        if (target != null && target.path() != null && !target.path().isBlank()) { // 只有真实 Controller 文件目标才写入 path。
            arguments.put("path", target.path()); // path 只允许项目源码相对路径或类名定位结果，不写入 /api endpoint。
        }
        if (endpoint != null && !endpoint.isBlank()) { // 指定接口时才传 endpoint。
            arguments.put("endpoint", endpoint); // 限定到该接口路径。
        }
        if (!endpointQuery && methodName != null && !methodName.isBlank()) { // 仅非 endpoint 查询才附加 methodName，endpoint 查询保持全项目扫描不被方法名约束。
            arguments.put("methodName", methodName); // 限定到该 Controller 方法。
        }
        arguments.put("maxDepth", maxDepth); // 分析深度。
        arguments.put("includeSnippet", true); // 默认带调用表达式片段。
        arguments.put("maxItems", FORCE_CONTROLLER_SERVICE_CHAIN_MAX_ITEMS); // 强制路由使用默认项数上限。
        String routeReason = endpoint != null && !endpoint.isBlank() && (target == null || target.path() == null || target.path().isBlank())
                ? ROUTE_REASON_FORCE_ANALYZE_CONTROLLER_SERVICE_CHAIN_BY_ENDPOINT
                : target != null && target.byFocus()
                ? ROUTE_REASON_FORCE_ANALYZE_CONTROLLER_SERVICE_CHAIN_BY_FOCUS
                : ROUTE_REASON_FORCE_ANALYZE_CONTROLLER_SERVICE_CHAIN; // 区分 endpoint-only、焦点目标和显式 Controller 目标。
        String toolResult = executeToolWithLog(controllerServiceChainTool, arguments, requestContext,
                CALL_SOURCE_FORCE_ROUTE, routeReason); // 执行工具并记录tool_call_log。
        logToolResult(toolResult); // 工具结果只打preview，不打印完整链路JSON。
        saveProjectFileFocusFromControllerServiceChainResult(toolResult, requestContext); // 成功分析后刷新projectFileFocus元信息。

        String argumentsJson = serializeArguments(arguments); // 复用日志入参序列化，保证最终回答阶段明确当前path。
        ObjectNode streamRequest = buildToolFinalAnswerRequest(userMessage, ANALYZE_CONTROLLER_SERVICE_CHAIN_TOOL_NAME, argumentsJson, toolResult); // 最终回答只保留本轮工具结果。
        streamSafeToolFinalAnswer(ANALYZE_CONTROLLER_SERVICE_CHAIN_TOOL_NAME, userMessage, argumentsJson, toolResult,
                streamRequest, callback, streamErrorNotified); // 统一检测并拦截DSML/tool_calls泄漏。
    }

    private void streamWithForcedToolServiceChain(String userMessage,
                                                  String memorySummary,
                                                  List<ToolChatHistoryMessage> historyMessages,
                                                  ToolCallingRequestContext requestContext,
                                                  ToolCallingStreamCallback callback,
                                                  boolean[] streamErrorNotified) { // 流式接口的后端强制 analyzeToolServiceChain 闭环。
        log.info("[ToolChatStream] force execute tool: {}", ANALYZE_TOOL_SERVICE_CHAIN_TOOL_NAME); // 打印强制执行的工具名。
        log.info("[ToolChatStream] rag search status: {}", RagSearchStatus.NOT_SEARCHED); // Tool→业务组件链路分支没有执行ragSearch。
        AiTool toolServiceChainTool = toolRegistry.getTool(ANALYZE_TOOL_SERVICE_CHAIN_TOOL_NAME); // 从ToolRegistry获取新工具。
        if (toolServiceChainTool == null) { // 工具未注册时不能伪造分析结果。
            callback.onToken("系统错误：analyzeToolServiceChain 工具未注册。"); // 通过SSE返回明确错误。
            callback.onComplete(); // 结束本轮SSE。
            return; // 工具缺失时不再继续。
        }

        AnalyzeToolTarget target = resolveToolTargetFromMessage(userMessage, requestContext); // 解析 path/toolName/className 或 AI Tool 文件焦点。
        if (target == null || !target.hasAnyTarget()) { // 没有可执行 Tool 目标时不乱猜。
            callback.onToken(buildToolServiceChainTargetMissingPrompt(requestContext)); // 提示指定 Tool 类名、路径或 toolName。
            callback.onComplete(); // 结束本轮SSE。
            return; // 不执行工具。
        }

        String methodName = extractToolServiceEntryMethodName(userMessage); // 支持用户指定 execute 之外的入口方法。
        int maxDepth = isToolServiceDepthIntent(userMessage) ? 2 : FORCE_TOOL_SERVICE_CHAIN_MAX_DEPTH; // 数据库/Mapper/ServiceImpl 等问题使用二层分析。
        log.info("[ToolChatStream] force analyzeToolServiceChain path: {}, toolName: {}, className: {}, byFocus: {}, methodName: {}, maxDepth: {}",
                target.path(), target.toolName(), target.className(), target.byFocus(), methodName, maxDepth); // 只打印相对路径和目标名。
        ObjectNode arguments = objectMapper.createObjectNode(); // 构造工具入参。
        if (target.path() != null && !target.path().isBlank()) { // 只有真实 Tool 文件路径或焦点路径才写入 path。
            arguments.put("path", target.path()); // path 必须是 workspace 相对路径，不用 toolName 硬拼。
        }
        if (target.toolName() != null && !target.toolName().isBlank()) { // 有明确 toolName 时写入。
            arguments.put("toolName", target.toolName()); // 交给工具全 workspace 扫描真实 Tool 文件。
        }
        if (target.className() != null && !target.className().isBlank()) { // 有明确类名时写入。
            arguments.put("className", target.className()); // 交给工具按类名定位真实文件。
        }
        arguments.put("methodName", methodName); // 默认 execute，保证日志参数明确。
        arguments.put("maxDepth", maxDepth); // 分析深度。
        arguments.put("includeSnippet", true); // 默认带少量调用表达式片段。
        arguments.put("maxItems", FORCE_TOOL_SERVICE_CHAIN_MAX_ITEMS); // 强制路由使用默认项数上限。
        String routeReason = target.byFocus()
                ? ROUTE_REASON_FORCE_ANALYZE_TOOL_SERVICE_CHAIN_BY_FOCUS
                : target.path() == null && target.toolName() != null && !target.toolName().isBlank()
                ? ROUTE_REASON_FORCE_ANALYZE_TOOL_SERVICE_CHAIN_BY_TOOL_NAME
                : ROUTE_REASON_FORCE_ANALYZE_TOOL_SERVICE_CHAIN; // 区分焦点、toolName 和显式 Tool 目标。
        String toolResult = executeToolWithLog(toolServiceChainTool, arguments, requestContext,
                CALL_SOURCE_FORCE_ROUTE, routeReason); // 执行工具并记录 tool_call_log。
        logToolResult(toolResult); // 工具结果只打 preview，不打印完整链路 JSON。
        saveProjectFileFocusFromToolServiceChainResult(toolResult, requestContext); // 成功分析后刷新 projectFileFocus。

        String argumentsJson = serializeArguments(arguments); // 复用日志入参序列化，保证最终回答阶段明确当前目标。
        ObjectNode streamRequest = buildToolFinalAnswerRequest(userMessage, ANALYZE_TOOL_SERVICE_CHAIN_TOOL_NAME, argumentsJson, toolResult); // 最终回答只保留本轮工具结果。
        streamSafeToolFinalAnswer(ANALYZE_TOOL_SERVICE_CHAIN_TOOL_NAME, userMessage, argumentsJson, toolResult,
                streamRequest, callback, streamErrorNotified); // 统一检测并拦截DSML/tool_calls泄漏。
    }

    private void streamWithForcedSseEventChain(String userMessage,
                                               String memorySummary,
                                               List<ToolChatHistoryMessage> historyMessages,
                                               ToolCallingRequestContext requestContext,
                                               ToolCallingStreamCallback callback,
                                               boolean[] streamErrorNotified) { // 流式接口的后端强制 analyzeSseEventChain 闭环。
        log.info("[ToolChatStream] force execute tool: {}", ANALYZE_SSE_EVENT_CHAIN_TOOL_NAME); // 打印强制执行的工具名。
        log.info("[ToolChatStream] rag search status: {}", RagSearchStatus.NOT_SEARCHED); // SSE 事件链路分支没有执行ragSearch。
        AiTool sseEventChainTool = toolRegistry.getTool(ANALYZE_SSE_EVENT_CHAIN_TOOL_NAME); // 从ToolRegistry获取新工具。
        if (sseEventChainTool == null) { // 工具未注册时不能伪造分析结果。
            callback.onToken("系统错误：analyzeSseEventChain 工具未注册。"); // 通过SSE返回明确错误。
            callback.onComplete(); // 结束本轮SSE。
            return; // 工具缺失时不再继续。
        }

        String endpoint = projectCodeTargetResolver.extractEndpoint(userMessage); // 提取 SSE 接口路径（明确时全项目扫描，不绑定 focus）。
        String eventName = extractSseEventName(userMessage); // 提取 SSE 事件名。
        String frontendKeyword = extractSseFrontendKeyword(userMessage); // 提取前端关键词。
        String methodName = extractCallChainMethodName(userMessage); // 复用方法名提取。
        boolean explicitTarget = endpoint != null || eventName != null || frontendKeyword != null; // 是否有明确 SSE 目标。
        String focusPath = (!explicitTarget && isSseFocus(requestContext))
                ? requestContext.getProjectFileFocus().getPath()
                : null; // 仅在无明确目标且焦点是 SSE 文件时使用 projectFileFocus.path。
        if (!explicitTarget && (focusPath == null || focusPath.isBlank())) { // 既无明确目标也无可用 SSE 焦点。
            callback.onToken("请提供 SSE 接口路径、事件名、前端关键词或相关文件路径，例如：分析 /chat/message 的 SSE 链路、summary_result 事件前端在哪里接收。"); // 不乱猜，提示用户补充。
            callback.onComplete(); // 结束本轮SSE。
            return; // 不执行工具。
        }

        int maxDepth = isSseDepthIntent(userMessage) ? 2 : FORCE_SSE_EVENT_CHAIN_MAX_DEPTH; // ServiceImpl/Tool/内部等意图使用二层分析。
        log.info("[ToolChatStream] force analyzeSseEventChain endpoint: {}, eventName: {}, frontendKeyword: {}, focusPath: {}, maxDepth: {}",
                endpoint, eventName, frontendKeyword, focusPath, maxDepth); // 只打印过滤条件和相对路径。
        ObjectNode arguments = objectMapper.createObjectNode(); // 构造工具入参。
        if (focusPath != null && !focusPath.isBlank()) { // 仅无明确目标且 SSE 焦点存在时写入 path。
            arguments.put("path", focusPath); // path 为 workspace 相对路径。
        }
        if (endpoint != null && !endpoint.isBlank()) { // 写入 endpoint。
            arguments.put("endpoint", endpoint); // 接口路径。
        }
        if (eventName != null && !eventName.isBlank()) { // 写入 eventName。
            arguments.put("eventName", eventName); // 事件名。
        }
        if (frontendKeyword != null && !frontendKeyword.isBlank()) { // 写入 frontendKeyword。
            arguments.put("frontendKeyword", frontendKeyword); // 前端关键词。
        }
        if (methodName != null && !methodName.isBlank()) { // 写入 methodName。
            arguments.put("methodName", methodName); // 后端方法名。
        }
        arguments.put("maxDepth", maxDepth); // 分析深度。
        arguments.put("includeSnippet", true); // 默认带少量片段。
        arguments.put("maxItems", FORCE_SSE_EVENT_CHAIN_MAX_ITEMS); // 强制路由使用默认项数上限。
        String toolResult = executeToolWithLog(sseEventChainTool, arguments, requestContext,
                CALL_SOURCE_FORCE_ROUTE, ROUTE_REASON_FORCE_ANALYZE_SSE_EVENT_CHAIN); // 执行工具并记录 tool_call_log。
        logToolResult(toolResult); // 工具结果只打 preview，不打印完整链路 JSON。

        String argumentsJson = serializeArguments(arguments); // 复用日志入参序列化，保证最终回答阶段明确当前目标。
        ObjectNode streamRequest = buildToolFinalAnswerRequest(userMessage, ANALYZE_SSE_EVENT_CHAIN_TOOL_NAME, argumentsJson, toolResult); // 最终回答只保留本轮工具结果。
        streamSafeToolFinalAnswer(ANALYZE_SSE_EVENT_CHAIN_TOOL_NAME, userMessage, argumentsJson, toolResult,
                streamRequest, callback, streamErrorNotified); // 统一检测并拦截DSML/tool_calls泄漏。
    }
    ...
    private void saveProjectFileFocusFromCallChainResult(String toolResult,
                                                         ToolCallingRequestContext requestContext) { // 从 analyzeCallChain 结果中提取安全元信息并保存 projectFileFocus。
        if (conversationFocusService == null || requestContext == null) { // 旧入口没有上下文或焦点服务时跳过。
            return; // 不影响工具主流程。
        }
        try {
            JsonNode resultNode = objectMapper.readTree(emptyJsonObjectIfBlank(toolResult)); // 解析 analyzeCallChain 返回 JSON。
            if (!resultNode.path("success").asBoolean(false)
                    || !"call_chain_analysis".equals(resultNode.path("type").asText(""))) { // 只保存成功的调用链分析结果。
                return; // 失败结果不切换焦点。
            }
            String path = resultNode.path("path").asText(""); // 读取 workspace 相对路径。
            if (path == null || path.isBlank()) { // 没有相对路径时不能保存焦点。
            ...
            conversationFocusService.saveProjectFileFocus(requestContext.getUserId(),
                    ...
                    truncated,
                    "CALL_CHAIN",
                    ANALYZE_CALL_CHAIN_TOOL_NAME); // 写入 Redis projectFileFocus，不保存文件内容。
            requestContext.setProjectFileFocus(conversationFocusService.getProjectFileFocus(requestContext.getUserId(),
                    requestContext.getConversationId())); // 同一轮内刷新上下文焦点。
        } catch (Exception e) {
            log.warn("[ProjectFileFocus] save from analyzeCallChain result failed, userId: {}, conversationId: {}, error: {}",
                    requestContext.getUserId(), requestContext.getConversationId(), e.getMessage(), e); // 只记录归属ID和错误。
        }
    }

    private void saveProjectFileFocusFromControllerServiceChainResult(String toolResult,
                                                                      ToolCallingRequestContext requestContext) { // 从 analyzeControllerServiceChain 结果中提取安全元信息并保存 projectFileFocus。
        if (conversationFocusService == null || requestContext == null) { // 旧入口没有上下文或焦点服务时跳过。
            return; // 不影响工具主流程。
        }
        try {
            JsonNode resultNode = objectMapper.readTree(emptyJsonObjectIfBlank(toolResult)); // 解析返回 JSON。
            if (!resultNode.path("success").asBoolean(false)
                    || !"controller_service_chain_analysis".equals(resultNode.path("type").asText(""))) { // 只保存成功的链路分析结果。
                return; // 失败结果不切换焦点。
            }
            String path = resultNode.path("path").asText(""); // 读取 workspace 相对路径。
            if (path == null || path.isBlank()) { // 没有相对路径时不能保存焦点。
            ...
            conversationFocusService.saveProjectFileFocus(requestContext.getUserId(),
                    ...
                    fileSize,
                    truncated,
                    "CONTROLLER_SERVICE_CHAIN",
                    ANALYZE_CONTROLLER_SERVICE_CHAIN_TOOL_NAME); // 写入 Redis projectFileFocus，不保存文件内容。
            requestContext.setProjectFileFocus(conversationFocusService.getProjectFileFocus(requestContext.getUserId(),
                    requestContext.getConversationId())); // 同一轮内刷新上下文焦点。
        } catch (Exception e) {
            log.warn("[ProjectFileFocus] save from analyzeControllerServiceChain result failed, userId: {}, conversationId: {}, error: {}",
                    requestContext.getUserId(), requestContext.getConversationId(), e.getMessage(), e); // 只记录归属ID和错误。
        }
    }

    private void saveProjectFileFocusFromToolServiceChainResult(String toolResult,
                                                                ToolCallingRequestContext requestContext) { // 从 analyzeToolServiceChain 结果中提取安全元信息并保存 projectFileFocus。
        if (conversationFocusService == null || requestContext == null) { // 旧入口没有上下文或焦点服务时跳过。
            return; // 不影响工具主流程。
        }
        try {
            JsonNode resultNode = objectMapper.readTree(emptyJsonObjectIfBlank(toolResult)); // 解析返回 JSON。
            if (!resultNode.path("success").asBoolean(false)
                    || !"tool_service_chain_analysis".equals(resultNode.path("type").asText(""))) { // 只保存成功的 Tool 链路分析结果。
                return; // 失败结果不切换焦点。
            }
            String path = resultNode.path("path").asText(""); // 读取 workspace 相对路径。
            if (path == null || path.isBlank()) { // 没有相对路径时不能保存焦点。
            ...
            conversationFocusService.saveProjectFileFocus(requestContext.getUserId(),
                    ...
                    truncated,
                    "TOOL_SERVICE_CHAIN",
                    ANALYZE_TOOL_SERVICE_CHAIN_TOOL_NAME); // 写入 Redis projectFileFocus，不保存文件内容。
            requestContext.setProjectFileFocus(conversationFocusService.getProjectFileFocus(requestContext.getUserId(),
                    requestContext.getConversationId())); // 同一轮内刷新上下文焦点。
        } catch (Exception e) {
            log.warn("[ProjectFileFocus] save from analyzeToolServiceChain result failed, userId: {}, conversationId: {}, error: {}",
                    requestContext.getUserId(), requestContext.getConversationId(), e.getMessage(), e); // 只记录归属ID和错误。
        }
    }
    ...
    private ObjectNode buildToolFinalAnswerRequest(String userMessage,
        ...
        if (ANALYZE_CODE_TOOL_NAME.equals(toolName)) { // analyzeCode追加专项规则。
            messages.add(buildMessage("system", ANALYZE_CODE_FINAL_ANSWER_SYSTEM_PROMPT)); // 强制围绕当前path总结代码结构。
        }
        if (ANALYZE_CALL_CHAIN_TOOL_NAME.equals(toolName)) { // analyzeCallChain追加专项规则。
            messages.add(buildMessage("system", ANALYZE_CALL_CHAIN_FINAL_ANSWER_SYSTEM_PROMPT)); // 强制围绕当前path总结调用关系。
        }
        if (ANALYZE_CONTROLLER_SERVICE_CHAIN_TOOL_NAME.equals(toolName)) { // analyzeControllerServiceChain追加专项规则。
            messages.add(buildMessage("system", ANALYZE_CONTROLLER_SERVICE_CHAIN_FINAL_ANSWER_SYSTEM_PROMPT)); // 强制围绕当前Controller总结接口链路。
        }
        if (ANALYZE_TOOL_SERVICE_CHAIN_TOOL_NAME.equals(toolName)) { // analyzeToolServiceChain追加专项规则。
            messages.add(buildMessage("system", ANALYZE_TOOL_SERVICE_CHAIN_FINAL_ANSWER_SYSTEM_PROMPT)); // 强制围绕当前AI Tool总结业务组件链路。
        }
        if (ANALYZE_SSE_EVENT_CHAIN_TOOL_NAME.equals(toolName)) { // analyzeSseEventChain追加专项规则。
            messages.add(buildMessage("system", ANALYZE_SSE_EVENT_CHAIN_FINAL_ANSWER_SYSTEM_PROMPT)); // 强制围绕本轮 SSE 链路结果回答。
        }
    ...
    private String resolveKnownProjectToolNameFromFileName(String fileName) { // 从已知Tool文件名映射规范toolName。
        ...
        if ("AnalyzeCallChainTool.java".equals(fileName)) { // 项目代码轻量调用链工具。
            return ANALYZE_CALL_CHAIN_TOOL_NAME; // 返回analyzeCallChain。
        }
        if ("AnalyzeControllerServiceChainTool.java".equals(fileName)) { // Controller→Service 专项链路工具。
            return ANALYZE_CONTROLLER_SERVICE_CHAIN_TOOL_NAME; // 返回analyzeControllerServiceChain。
        }
        if ("AnalyzeToolServiceChainTool.java".equals(fileName)) { // AI Tool→业务组件专项链路工具。
            return ANALYZE_TOOL_SERVICE_CHAIN_TOOL_NAME; // 返回analyzeToolServiceChain。
        }
    ...
    private String resolveToolType(String toolName) { // 按工具名映射日志工具类型。
        ...
        if (ANALYZE_CALL_CHAIN_TOOL_NAME.equals(toolName)) { // analyzeCallChain日志类型。
            return TOOL_TYPE_PROJECT; // 返回PROJECT。
        }
        if (ANALYZE_CONTROLLER_SERVICE_CHAIN_TOOL_NAME.equals(toolName)) { // analyzeControllerServiceChain日志类型。
            return TOOL_TYPE_PROJECT; // 返回PROJECT。
        }
        if (ANALYZE_TOOL_SERVICE_CHAIN_TOOL_NAME.equals(toolName)) { // analyzeToolServiceChain日志类型。
            return TOOL_TYPE_PROJECT; // 返回PROJECT。
        }
        if (ANALYZE_SSE_EVENT_CHAIN_TOOL_NAME.equals(toolName)) { // analyzeSseEventChain日志类型。
            return TOOL_TYPE_PROJECT; // 返回PROJECT。
        }
    ...
    private boolean shouldForceReadProjectFile(String message,
        ...
        if (isAnalyzeCodeIntent(normalizedMessage) && !containsAny(normalizedMessage, READ_PROJECT_FILE_FULL_KEYWORDS)) { // 结构/字段/方法分析交给 analyzeCode。
        ...
        if (isControllerServiceChainIntent(normalizedMessage) && extractControllerEndpoint(message) != null) { // Controller→Service endpoint-only 请求不能把 /api/xxx 当作项目文件路径。
            return false; // 交给 analyzeControllerServiceChain 的 endpoint 参数处理。
        }
        if (isToolServiceChainIntent(normalizedMessage)) { // Tool→业务组件链路请求不能被 readProjectFile 抢占。
            return false; // 交给 analyzeToolServiceChain 处理。
        }
    ...
    private AnalyzeCodeTarget resolveAnalyzeCodeTargetFromMessage(String message,
        ...
        if (explicitProjectPath != null && !explicitProjectPath.isBlank()) { // 用户显式给了路径/文件名/类名。
            return new AnalyzeCodeTarget(explicitProjectPath, false); // 显式目标优先。
        }
        ...
        if (focus != null && focus.getPath() != null && !focus.getPath().isBlank()) { // 有最近项目源码文件焦点。
            return new AnalyzeCodeTarget(focus.getPath(), true); // 仅指代追问场景用焦点文件分析。
        }
        return null; // 无目标时不猜测。
    }
    ...
    private AnalyzeCodeTarget resolveControllerTargetFromMessage(String message,
        ...
        if (explicitProjectPath != null && !explicitProjectPath.isBlank()
            ...
            return new AnalyzeCodeTarget(explicitProjectPath, false); // 显式 Controller 目标优先。
        }
        ConversationFocusContext focus = requestContext == null ? null : requestContext.getProjectFileFocus(); // 读取项目源码文件焦点。
        if (focus != null && focus.getPath() != null && !focus.getPath().isBlank()
            ...
            return new AnalyzeCodeTarget(focus.getPath(), true); // 返回焦点 Controller。
        }
    ...
    private boolean shouldForceAnalyzeToolServiceChain(String message,
        ...
        if (containsAny(normalizedMessage, TOOL_SERVICE_CHAIN_ORDINARY_CALL_CHAIN_KEYWORDS)) { // 用户明确说普通调用链时交给 analyzeCallChain。
            return false; // 返回false。
        }
        boolean hasExplicitToolTarget = hasExplicitToolTarget(message); // 是否给了 Tool 类名、路径或已知 toolName。
        boolean hasToolFocus = isProjectFileFocusAiTool(requestContext); // 当前 projectFileFocus 是否指向 AI Tool 文件。
        ...
        if (isToolServiceChainIntent(normalizedMessage) && (hasExplicitToolTarget || hasToolFocus || hasExplicitToolWord)) { // Tool follow-up 意图 + Tool 目标信号。
            return true; // 触发 analyzeToolServiceChain。
        }
    ...
    private AnalyzeToolTarget resolveToolTargetFromMessage(String message,
                                                           ToolCallingRequestContext requestContext) { // 解析 analyzeToolServiceChain 的 path/toolName/className 目标。
        ...
        if (explicitProjectPath != null && !explicitProjectPath.isBlank()
            ...
            return new AnalyzeToolTarget(explicitProjectPath, null, deriveClassNameFromProjectPath(explicitProjectPath), false); // 显式 Tool 文件优先。
        }
        ...
        if ((toolName != null && !toolName.isBlank()) || (className != null && !className.isBlank())) { // 有明确 toolName 或 className。
            return new AnalyzeToolTarget(null, toolName, className, false); // 交给工具全 workspace 定位真实 Tool 文件。
        }
        ...
        if (isProjectFileFocusAiTool(requestContext)) { // 只允许 AI Tool 文件焦点参与 Tool 专项链路。
            ...
            return new AnalyzeToolTarget(focus.getPath(), focusToolName, deriveClassNameFromProjectPath(focus.getPath()), true); // 返回焦点 Tool。
        }
    ...
    private boolean isProjectFileFocusAiTool(ToolCallingRequestContext requestContext) { // 判断当前 projectFileFocus 是否是 AI Tool 文件。
        ...
        return looksLikeToolFileTarget(focus.getPath()); // Tool 模块不读取源码内容，真实安全校验和 AI Tool 特征校验交给 AnalyzeToolServiceChainTool。
    }
    ...
    private boolean isSseFocus(ToolCallingRequestContext requestContext) { // 判断当前 projectFileFocus 是否是前端 SSE 文件或后端 SSE Controller/Service。
        ...
        String path = focus.getPath().toLowerCase(Locale.ROOT); // 焦点相对路径小写。
        return path.endsWith(".vue") || path.endsWith(".ts") || path.endsWith(".tsx") || path.endsWith(".js") || path.endsWith(".jsx") // 前端 SSE 文件。
                || path.contains("/controller/") || path.endsWith("controller.java") // 后端 Controller。
                || path.endsWith("serviceimpl.java") || path.contains("chatmessage"); // 后端 SSE Service。
    }
    ...
    private boolean shouldForceAnalyzeCallChain(String message,
        ...
        return target != null && target.path() != null && !target.path().isBlank(); // 必须有可执行目标才强制 analyzeCallChain。
    }
    ...
    private String resolveKnownProjectFilePath(String fileName) { // 将已知项目Tool源码文件名映射为相对workspace路径。
        ...
        return switch (fileName) {
            ...
            case "AnalyzeCallChainTool.java" -> "Tech-Brain-Agent/src/main/java/com/agent/tool/project/AnalyzeCallChainTool.java";
            case "AnalyzeControllerServiceChainTool.java" -> "Tech-Brain-Agent/src/main/java/com/agent/tool/project/AnalyzeControllerServiceChainTool.java";
            case "AnalyzeToolServiceChainTool.java" -> "Tech-Brain-Agent/src/main/java/com/agent/tool/project/AnalyzeToolServiceChainTool.java";
    ...
    private record AnalyzeCodeTarget(String path, boolean byFocus) { // analyzeCode 强制路由目标。
    }

    private record AnalyzeToolTarget(String path, String toolName, String className, boolean byFocus) { // analyzeToolServiceChain 强制路由目标。
