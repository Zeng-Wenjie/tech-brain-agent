...
public class ToolCallingChatServiceImpl implements ToolCallingChatService { // 默认Tool Calling聊天编排实现。
    ...
    private static final String SYSTEM_PROMPT = "你是 Tech-Brain 项目的 AI 助手。" + // 边界优化版system prompt，定义助手身份。
            ...
            "\n6. analyzeCode：统一项目代码分析工具，通过 analysisType 支持 STRUCTURE、CALL_CHAIN、CONTROLLER_SERVICE、TOOL_SERVICE、SSE_EVENT_CHAIN，不返回完整源码，不修改项目文件。" +
            ...
            "\n- 如果用户要搜索代码位置，应调用 searchCode；如果用户要分析项目源码结构、调用链、Controller→Service、Tool→Service 或 SSE 链路，应调用 analyzeCode；如果用户要读取指定项目代码文件，应调用 readProjectFile。" +
    ...
    private ObjectNode buildForcedAnalyzeCodeArguments(String userMessage,
        ...
        arguments.put("includeSnippet", true); // 默认返回少量片段。
