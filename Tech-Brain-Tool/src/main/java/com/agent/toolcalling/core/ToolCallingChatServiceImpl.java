...
public class ToolCallingChatServiceImpl implements ToolCallingChatService { // 默认Tool Calling聊天编排实现。
    ...
    private boolean shouldForceSummarizeArticle(String currentMessage) { // 判断用户是否明确要求总结某篇文章/笔记。
        ...
        boolean hasSummaryIntent = containsAny(currentMessage, FORCE_SUMMARY_KEYWORDS); // 只基于当前输入判断是否有总结意图。
        if (!hasSummaryIntent) { // 没有总结、摘要、要点、面试话术等表达。
            return false; // 不强制summarizeArticle。
        }
        return containsAny(currentMessage, FORCE_SUMMARY_TARGET_KEYWORDS); // 必须同时指向笔记/文章/这篇/刚才/第几篇等目标。
    }
    ...
    private boolean containsAny(String content, String[] keywords) { // 判断文本是否包含任意关键词。
        ...
        for (String keyword : keywords) { // 遍历关键词。
            if (keyword != null && !keyword.isBlank() && content.contains(keyword)) { // 命中关键词。
                return true; // 返回true。
            }
        }
