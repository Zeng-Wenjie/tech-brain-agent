...
public class ToolCallingChatServiceImpl implements ToolCallingChatService { // 默认Tool Calling聊天编排实现。
    ...
    private static final String SYSTEM_PROMPT = "你是 Tech-Brain 项目的 AI 助手。" + // 边界优化版system prompt，定义助手身份。
            ...
            "\n10. readProjectFile：用于读取项目 workspace 内指定代码/文本文件内容，必须传相对 workspace 的文件路径，禁止读取敏感文件和 workspace 外路径。" +
            "\n\n调用 ragSearch 的规则：" +
    ...
    public void chatStream(String currentMessage,
        ...
        try { // 统一兜底Tool Calling流式编排中的异常。
            ...
            boolean forceControllerServiceChain = !forceSummarizeArticle && !forceReadProjectFile && shouldForceAnalyzeControllerServiceChain(userMessage, requestContext); // Controller→Service 接口链路优先于普通调用链。
            log.info("[ToolChatStream] force analyzeControllerServiceChain: {}", forceControllerServiceChain); // 打印强制 Controller→Service 链路路由判断结果。
            boolean forceToolServiceChain = !forceSummarizeArticle && !forceReadProjectFile && !forceControllerServiceChain && shouldForceAnalyzeToolServiceChain(userMessage, requestContext); // Tool→业务组件链路优先于普通调用链。
            log.info("[ToolChatStream] force analyzeToolServiceChain: {}", forceToolServiceChain); // 打印强制 Tool→业务组件链路路由判断结果。
            boolean forceAnalyzeCallChain = !forceSummarizeArticle && !forceReadProjectFile && !forceControllerServiceChain && !forceToolServiceChain && shouldForceAnalyzeCallChain(userMessage, requestContext); // 调用链/调用关系/依赖分析优先于单文件结构分析。
            log.info("[ToolChatStream] force analyzeCallChain: {}", forceAnalyzeCallChain); // 打印强制调用链分析路由判断结果。
            boolean forceAnalyzeCode = !forceSummarizeArticle && !forceReadProjectFile && !forceControllerServiceChain && !forceToolServiceChain && !forceAnalyzeCallChain && shouldForceAnalyzeCode(userMessage, requestContext); // 结构/方法/字段/接口分析优先于项目文件焦点读取。
            log.info("[ToolChatStream] force analyzeCode: {}", forceAnalyzeCode); // 打印强制代码结构分析路由判断结果。
            boolean forceProjectFileFocus = !forceSummarizeArticle && !forceReadProjectFile && !forceControllerServiceChain && !forceToolServiceChain && !forceAnalyzeCallChain && !forceAnalyzeCode && shouldUseProjectFileFocus(userMessage, requestContext); // 项目源码文件追问优先使用最近 readProjectFile 焦点。
            ...
            boolean forceReadFile = !forceReadProjectFile && !forceControllerServiceChain && !forceToolServiceChain && !forceAnalyzeCallChain && !forceAnalyzeCode && !forceProjectFileFocus && !promptProjectFileFocusMissing && shouldForceReadFile(userMessage, requestContext); // 明确要求读取/分析当前附件时由后端强制readFile，避免模型漏调工具。
            log.info("[ToolChatStream] force readFile: {}", forceReadFile); // 打印强制readFile路由判断结果。
            boolean forceListProjectTree = !forceReadProjectFile && !forceControllerServiceChain && !forceToolServiceChain && !forceAnalyzeCallChain && !forceAnalyzeCode && !forceProjectFileFocus && !promptProjectFileFocusMissing && !forceReadFile && shouldForceListProjectTree(userMessage); // 用户要求查看项目目录结构时强制执行listProjectTree。
            log.info("[ToolChatStream] force listProjectTree: {}", forceListProjectTree); // 打印强制目录树路由判断结果。
            boolean forceSearchCode = !forceReadProjectFile && !forceControllerServiceChain && !forceToolServiceChain && !forceAnalyzeCallChain && !forceAnalyzeCode && !forceProjectFileFocus && !promptProjectFileFocusMissing && !forceReadFile && !forceListProjectTree && shouldForceSearchCode(userMessage); // 用户要求定位类、方法、注解或关键词位置时强制执行searchCode。
