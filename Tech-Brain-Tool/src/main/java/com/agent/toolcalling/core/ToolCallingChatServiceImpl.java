...
public class ToolCallingChatServiceImpl implements ToolCallingChatService { // 默认Tool Calling聊天编排实现。
    ...
    private static final String SYSTEM_PROMPT = "你是 Tech-Brain 项目的 AI 助手。" + // 边界优化版system prompt，定义助手身份。
            ...
            "\n6. analyzeCode：统一项目代码分析工具，通过 analysisType 支持 STRUCTURE、CALL_CHAIN、CONTROLLER_SERVICE、TOOL_SERVICE、SSE_EVENT_CHAIN、EXPLANATION、RISK，不返回完整源码，不修改项目文件。" +
            ...
            "\n- analyzeCode 只返回结构化静态分析结果，不返回完整源码，不做完整精准调用图，不做测试步骤、开发日志保存或 patch，不修改前端或后端业务代码；风险分析仅在 analysisType=RISK 时基于真实扫描证据输出风险点说明和简短建议，不生成修改方案。" +
    ...
    private static final String ANALYZE_CODE_FINAL_ANSWER_SYSTEM_PROMPT = "当前工具是 analyzeCode。" // analyzeCode 专项最终回答规则。
            ...
            + "\n不要输出完整源码，不要输出完整 result_json，不要生成测试步骤、修改方案或 patch（仅 analysisType=RISK 时可基于 result.risks/safePoints 输出风险点说明和简短建议），不要声称修改了文件。";
    ...
    public void chatStream(String currentMessage,
        ...
        try { // 统一兜底Tool Calling流式编排中的异常。
            ...
            boolean forceCodeRisk = !forceSummarizeArticle && !forceReadProjectFile && shouldForceCodeRisk(userMessage, requestContext); // 风险/隐患/安全问题优先于说明和其它分析，但 readProjectFile 仍最高。
            log.info("[ToolChatStream] force analyzeCode RISK: {}", forceCodeRisk); // 打印强制风险分析路由判断结果。
            boolean forceCodeExplanation = !forceSummarizeArticle && !forceReadProjectFile && !forceCodeRisk && shouldForceCodeExplanation(userMessage, requestContext); // 说明/解释/面试话术优先于普通分析，但 readProjectFile 和风险分析更高。
            log.info("[ToolChatStream] force analyzeCode EXPLANATION: {}", forceCodeExplanation); // 打印强制说明生成路由判断结果。
            boolean forceSseEventChain = !forceSummarizeArticle && !forceReadProjectFile && !forceCodeRisk && !forceCodeExplanation && shouldForceAnalyzeSseEventChain(userMessage, requestContext); // 前后端 SSE 事件链路优先于 Controller→Service 等链路。
            log.info("[ToolChatStream] force analyzeCode SSE_EVENT_CHAIN: {}", forceSseEventChain); // 打印强制 SSE 事件链路路由判断结果。
            boolean forceControllerServiceChain = !forceSummarizeArticle && !forceReadProjectFile && !forceCodeRisk && !forceCodeExplanation && !forceSseEventChain && shouldForceAnalyzeControllerServiceChain(userMessage, requestContext); // Controller→Service 接口链路优先于普通调用链。
            ...
            boolean forceToolServiceChain = !forceSummarizeArticle && !forceReadProjectFile && !forceCodeRisk && !forceCodeExplanation && !forceSseEventChain && !forceControllerServiceChain && shouldForceAnalyzeToolServiceChain(userMessage, requestContext); // Tool→业务组件链路优先于普通调用链。
            log.info("[ToolChatStream] force analyzeCode TOOL_SERVICE: {}", forceToolServiceChain); // 打印强制 Tool→业务组件链路路由判断结果。
            boolean forceAnalyzeCallChain = !forceSummarizeArticle && !forceReadProjectFile && !forceCodeRisk && !forceCodeExplanation && !forceSseEventChain && !forceControllerServiceChain && !forceToolServiceChain && shouldForceAnalyzeCallChain(userMessage, requestContext); // 调用链/调用关系/依赖分析优先于单文件结构分析。
            log.info("[ToolChatStream] force analyzeCode CALL_CHAIN: {}", forceAnalyzeCallChain); // 打印强制调用链分析路由判断结果。
            boolean forceAnalyzeCode = !forceSummarizeArticle && !forceReadProjectFile && !forceCodeRisk && !forceCodeExplanation && !forceSseEventChain && !forceControllerServiceChain && !forceToolServiceChain && !forceAnalyzeCallChain && shouldForceAnalyzeCode(userMessage, requestContext); // 结构/方法/字段/接口分析优先于项目文件焦点读取。
            log.info("[ToolChatStream] force analyzeCode: {}", forceAnalyzeCode); // 打印强制代码结构分析路由判断结果。
            boolean forceProjectFileFocus = !forceSummarizeArticle && !forceReadProjectFile && !forceCodeRisk && !forceCodeExplanation && !forceSseEventChain && !forceControllerServiceChain && !forceToolServiceChain && !forceAnalyzeCallChain && !forceAnalyzeCode && shouldUseProjectFileFocus(userMessage, requestContext); // 项目源码文件追问优先使用最近 readProjectFile 焦点。
    ...
    private ObjectNode buildForcedAnalyzeCodeArguments(String userMessage,
        ...
        arguments.put("includeSnippet", safeType != CodeAnalysisType.EXPLANATION
                && safeType != CodeAnalysisType.RISK); // 说明生成和风险说明默认不贴片段，其它分析默认返回少量片段。
        arguments.put("maxItems", resolveAnalyzeCodeMaxItems(safeType)); // 按类型复用原默认上限。
        if (safeType != CodeAnalysisType.EXPLANATION && safeType != CodeAnalysisType.RISK) { // 说明/风险生成不写 maxDepth，由内部 Generator/Analyzer 自行决定深度。
    ...
    private int resolveAnalyzeCodeMaxItems(CodeAnalysisType analysisType) { // 按分析类型复用旧默认 maxItems。
        if (analysisType == CodeAnalysisType.EXPLANATION || analysisType == CodeAnalysisType.RISK) { // 代码说明和风险说明。
