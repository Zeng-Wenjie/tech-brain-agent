...
public class ToolCallingChatServiceImpl implements ToolCallingChatService { // 默认Tool Calling聊天编排实现。
    ...
    private static final String SYSTEM_PROMPT = "你是 Tech-Brain 项目的 AI 助手。" + // 多工具版本system prompt，定义助手身份。
            "\n你可以根据用户问题选择合适的工具。" + // 允许后续扩展更多工具，不再把编排器绑定到ragSearch。
            "\n当用户要求“根据知识库”“根据我的笔记”“根据资料”“根据项目文档”回答时，你必须调用 ragSearch 工具。" + // 保持知识库问题必须走ragSearch。
            "\n如果用户只是普通闲聊，可以直接回答。" + // 普通闲聊允许不调用工具。
            "\n当工具返回了结果时，你必须优先根据工具结果回答。" + // 第二次回答必须基于tool result。
            "\n如果工具返回没有找到相关内容，请明确告诉用户没有找到相关资料，不要编造。" + // 无结果时禁止模型补编资料。
            "\n不要编造知识库内容。"; // 最终约束模型不要虚构知识库。
