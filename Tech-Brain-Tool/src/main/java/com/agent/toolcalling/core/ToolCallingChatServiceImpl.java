...
public class ToolCallingChatServiceImpl implements ToolCallingChatService { // 默认Tool Calling聊天编排实现。
    ...
    private static final String SYSTEM_PROMPT = "你是 Tech-Brain 项目的 AI 助手。" + // 边界优化版system prompt，定义助手身份。
            ...
            "\n- 如果用户要求读取附件正文，应调用 readFile 工具；如果 readFile 工具不可用，只能说明目前能看到文件名、类型、大小等元信息，不能读取正文。" +
    ...
    public void chatStream(String currentMessage,
        ...
        try { // 统一兜底Tool Calling流式编排中的异常。
            ...
            String answerMode = forceSummarizeArticle ? "force-summarizeArticle" : (forceRagSearch ? "force-ragSearch" : "model-routing"); // 记录本轮回答模式，不改变路由逻辑。
    ...
    private String buildAttachmentRagBlockedPrompt() {
        return "当前用户问题指向会话附件或刚上传的文件，不是知识库检索请求。" + // 附件RAG拦截后的额外边界。
                ...
                "\n如果用户要求读取、总结或分析文件正文，应调用 readFile 工具；如果 readFile 工具不可用，则说明当前只能看到文件名、类型、大小等元信息。"; // 不读取文件内容。
    }
    ...
    private String buildAttachedFilesPrompt(ToolCallingRequestContext requestContext) {
        if (!hasAttachedFiles(requestContext)) { // 没有附件时不生成提示。
            return null; // 返回空提示。
        }
        StringBuilder prompt = new StringBuilder("用户本轮附带了以下文件（仅文件元信息，后端尚未读取文件内容）："); // 明确只是元信息。
        List<ChatAttachedFileContext> attachedFiles = requestContext.getAttachedFiles(); // 读取本轮附件列表。
        for (int i = 0; i < attachedFiles.size(); i++) { // 按前端选择顺序输出。
            ...
            prompt.append("\n")
                    ...
                    .append(", 文件分类=")
                    .append(safeText(file.getFileType()))
                    .append(", MIME=")
                    .append(safeText(file.getMimeType()))
                    .append(", 大小=")
                    .append(formatFileSize(file.getFileSize())); // 只输出安全展示字段。
        }
        prompt.append("\n注意：以上只是文件元信息，不代表你已经读取了文件内容。") // 防止模型冒充已读文件。
                .append("\n当前项目尚未接入 readFile 文件读取工具，本轮不能读取或总结文件正文。") // 当前没有 readFileTool。
                .append("\n如果用户要求读取、总结或分析附件内容，请明确说明当前只能看到文件名、类型和大小等元信息，文件正文读取需等待 readFile 工具支持。") // 给模型安全答复策略。
    ...
    private boolean hasAttachedFiles(ToolCallingRequestContext requestContext) {
        return requestContext != null
                && requestContext.getAttachedFiles() != null
                && !requestContext.getAttachedFiles().isEmpty(); // 判断本轮是否携带附件元信息。
    }
    ...
    private boolean hasAttachmentContext(ToolCallingRequestContext requestContext) {
        return hasAttachedFiles(requestContext); // 当前上下文只看本轮attachedFiles，供基础规则复用。
    }
    ...
    private boolean shouldBlockRagForAttachmentIntent(String userMessage,
                                                      ToolCallingRequestContext requestContext) {
        return isFileAttachmentIntent(userMessage)
    ...
    private boolean shouldBlockRagForAttachmentIntent(String userMessage,
                                                      ...
                                                      List<ToolChatHistoryMessage> historyMessages) {
        return isFileAttachmentIntent(userMessage)
                && hasAttachmentContext(requestContext, historyMessages)
