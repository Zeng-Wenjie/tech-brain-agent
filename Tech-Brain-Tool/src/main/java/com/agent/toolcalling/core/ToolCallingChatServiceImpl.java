...
public class ToolCallingChatServiceImpl implements ToolCallingChatService { // 默认Tool Calling聊天编排实现。
    ...
    private static final int HISTORY_CONTEXT_LIMIT = 6; // Tool Calling最终回答阶段最多注入最近6条历史。
    ...
    public void chatStream(String currentMessage,
        ...
        try { // 统一兜底Tool Calling流式编排中的异常。
            ...
            if (!hasToolCall) { // 普通聊天不再一次性吐出第一次非流式结果，而是重新发起无tools的流式请求。
                ObjectNode noToolStreamRequest = buildNoToolStreamRequest(userMessage, normalizedHistoryMessages); // 第二次请求加入历史，但不设置tools。
    ...
    private ObjectNode buildNoToolStreamRequest(String userMessage, List<ToolChatHistoryMessage> historyMessages) { // 构造无工具场景的第二次流式请求。
        ...
        messages.add(buildMessage("system", NO_TOOL_SYSTEM_PROMPT)); // 明确本轮没有工具和知识库结果，避免普通回答伪装检索。
    ...
    private List<ToolChatHistoryMessage> normalizeHistoryMessages(List<ToolChatHistoryMessage> historyMessages) {
        ...
        normalizedMessages.sort(Comparator.comparing(
                ToolChatHistoryMessage::getCreateTime,
                Comparator.nullsFirst(LocalDateTime::compareTo))); // 按创建时间正序排列，null时间视为最早历史。
        if (normalizedMessages.size() <= HISTORY_CONTEXT_LIMIT) {
            return normalizedMessages; // 未超过窗口时直接返回正序历史。
        }
        return new ArrayList<>(normalizedMessages.subList(
                normalizedMessages.size() - HISTORY_CONTEXT_LIMIT,
                normalizedMessages.size())); // 超过窗口时只保留最近6条，并继续保持时间正序。
    }
    ...
    private String previewContent(String content) {
        ...
        return content.length() <= 80 ? content : content.substring(0, 80) + "..."; // 历史日志最多打印80字。
    }
