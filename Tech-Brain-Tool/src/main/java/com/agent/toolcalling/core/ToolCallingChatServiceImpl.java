...
public class ToolCallingChatServiceImpl implements ToolCallingChatService { // 默认Tool Calling聊天编排实现。
    ...
    public void chatStream(String currentMessage,
        ...
        try { // 统一兜底Tool Calling流式编排中的异常。
            ...
            ObjectNode secondRequest = buildSecondRequest(userMessage, normalizedMemorySummary, normalizedHistoryMessages, firstMessage, toolCallId, toolResult, ragSearchStatus); // 第二次请求加入长期记忆、历史、工具结果和RAG状态。
    ...
    private void streamWithForcedRagSearch(String userMessage,
        ...
        ObjectNode streamRequest = buildForcedRagAnswerRequest(userMessage, memorySummary, historyMessages, toolResult, ragSearchStatus); // 将长期记忆、历史和RAG状态作为辅助上下文，工具结果仍最高优先级。
    ...
    private ObjectNode buildForcedRagAnswerRequest(String userMessage,
                                                   ...
                                                   List<ToolChatHistoryMessage> historyMessages,
                                                   String toolResult,
                                                   RagSearchStatus ragSearchStatus) { // 构造后端强制ragSearch后的回答请求。
    ...
    private ObjectNode buildSecondRequest(String userMessage,
                                          ...
                                          String toolCallId,
                                          String toolResult,
                                          RagSearchStatus ragSearchStatus) { // 构造第二次DeepSeek请求。
