...
public class ToolCallingChatServiceImpl implements ToolCallingChatService { // 默认Tool Calling聊天编排实现。
    ...
    public String chat(String message) { // 接收用户消息并返回最终answer。
        ...
        boolean forceRagSearch = shouldForceRagSearch(userMessage); // 后端先判断是否必须走知识库，避免完全依赖模型tool_call。
        log.info("[ToolChat] force ragSearch: {}", forceRagSearch); // 打印同步接口的强制RAG路由结果。
        if (forceRagSearch) { // 用户明确要求基于知识库/笔记/资料回答时，直接执行ragSearch。
            return chatWithForcedRagSearch(userMessage); // 同步接口也走强制RAG，保证/api/ai/tool-chat行为稳定。
        }
    ...
    private String chatWithForcedRagSearch(String userMessage) { // 同步接口的后端强制ragSearch闭环。
        String query = buildRagQuery(userMessage); // 清理“请根据我的知识库回答”等前缀，让检索词更干净。
        log.info("[ToolChat] force execute tool: {}", RAG_SEARCH_TOOL_NAME); // 打印强制执行的工具名。
        log.info("[ToolChat] force ragSearch query: {}", query); // 打印最终进入ragSearch的检索词。
        AiTool ragTool = toolRegistry.getTool(RAG_SEARCH_TOOL_NAME); // 从注册中心获取业务模块提供的RagSearchTool。
        if (ragTool == null) { // ragSearch未注册说明Spring扫描或工具注册异常。
            return buildAnswerAndLog("系统错误：ragSearch 工具未注册。"); // 同步接口直接返回明确错误，避免继续请求模型编造。
        }
        ObjectNode arguments = objectMapper.createObjectNode(); // 构造ragSearch工具参数。
        arguments.put("query", query); // ragSearch只需要query参数。
        String toolResult = ragTool.execute(arguments); // 直接执行RagSearchTool，内部会调用AgentService.searchRagContents检索Milvus。
        log.info("[ToolChat] force ragSearch result: {}", toolResult); // 打印强制RAG返回内容，便于确认确实来自工具。
        ObjectNode forcedRequest = buildForcedRagAnswerRequest(userMessage, toolResult); // 把工具结果作为普通上下文发给模型生成最终回答。
        log.info("[ToolChat] before forced ragSearch DeepSeek call"); // 标记强制RAG后的同步模型调用开始。
        JsonNode response = deepSeekClient.chatCompletions(forcedRequest); // 同步生成最终答案，供/api/ai/tool-chat使用。
        log.info("[ToolChat] after forced ragSearch DeepSeek call"); // 标记强制RAG后的同步模型调用结束。
        JsonNode message = readFirstMessage(response); // 读取assistant message。
        return buildAnswerAndLog(readContent(message)); // 返回最终答案并保留final answer日志。
    }
