...
public class ToolCallingChatServiceImpl implements ToolCallingChatService { // 默认Tool Calling聊天编排实现。
    ...
    private static final String SYSTEM_PROMPT = "你是 Tech-Brain 项目的 AI 助手。" + // 边界优化版system prompt，定义助手身份。
            ...
            "\n9. readProjectFile：用于读取项目 workspace 内指定代码/文本文件内容，必须传相对 workspace 的文件路径，禁止读取敏感文件和 workspace 外路径。" +
            "\n\n调用 ragSearch 的规则：" +
    ...
    private static final Pattern CONTROLLER_ENDPOINT_PATTERN = Pattern.compile(
            "(/[A-Za-z0-9_{}\\-]+(?:/[A-Za-z0-9_{}\\-]+)*)"); // 从消息中提取 /api/xxx 形式的接口路径。
    ...
    public void chatStream(String currentMessage,
        ...
        try { // 统一兜底Tool Calling流式编排中的异常。
            ...
            boolean forceAnalyzeCallChain = !forceSummarizeArticle && !forceReadProjectFile && !forceControllerServiceChain && shouldForceAnalyzeCallChain(userMessage, requestContext); // 调用链/调用关系/依赖分析优先于单文件结构分析。
            log.info("[ToolChatStream] force analyzeCallChain: {}", forceAnalyzeCallChain); // 打印强制调用链分析路由判断结果。
            boolean forceAnalyzeCode = !forceSummarizeArticle && !forceReadProjectFile && !forceControllerServiceChain && !forceAnalyzeCallChain && shouldForceAnalyzeCode(userMessage, requestContext); // 结构/方法/字段/接口分析优先于项目文件焦点读取。
            log.info("[ToolChatStream] force analyzeCode: {}", forceAnalyzeCode); // 打印强制代码结构分析路由判断结果。
            boolean forceProjectFileFocus = !forceSummarizeArticle && !forceReadProjectFile && !forceControllerServiceChain && !forceAnalyzeCallChain && !forceAnalyzeCode && shouldUseProjectFileFocus(userMessage, requestContext); // 项目源码文件追问优先使用最近 readProjectFile 焦点。
            ...
            boolean forceReadFile = !forceReadProjectFile && !forceControllerServiceChain && !forceAnalyzeCallChain && !forceAnalyzeCode && !forceProjectFileFocus && !promptProjectFileFocusMissing && shouldForceReadFile(userMessage, requestContext); // 明确要求读取/分析当前附件时由后端强制readFile，避免模型漏调工具。
            log.info("[ToolChatStream] force readFile: {}", forceReadFile); // 打印强制readFile路由判断结果。
            boolean forceListProjectTree = !forceReadProjectFile && !forceControllerServiceChain && !forceAnalyzeCallChain && !forceAnalyzeCode && !forceProjectFileFocus && !promptProjectFileFocusMissing && !forceReadFile && shouldForceListProjectTree(userMessage); // 用户要求查看项目目录结构时强制执行listProjectTree。
            log.info("[ToolChatStream] force listProjectTree: {}", forceListProjectTree); // 打印强制目录树路由判断结果。
            boolean forceSearchCode = !forceReadProjectFile && !forceControllerServiceChain && !forceAnalyzeCallChain && !forceAnalyzeCode && !forceProjectFileFocus && !promptProjectFileFocusMissing && !forceReadFile && !forceListProjectTree && shouldForceSearchCode(userMessage); // 用户要求定位类、方法、注解或关键词位置时强制执行searchCode。
            log.info("[ToolChatStream] force searchCode: {}", forceSearchCode); // 打印强制代码搜索路由判断结果。
            boolean forceRagSearch = !forceSummarizeArticle && !forceReadProjectFile && !forceControllerServiceChain && !forceAnalyzeCallChain && !forceAnalyzeCode && !forceProjectFileFocus && !promptProjectFileFocusMissing && !forceReadFile && !forceListProjectTree && !forceSearchCode && shouldForceRagSearch(userMessage); // 只有没有更明确工具意图时才强制RAG。
    ...
    private AnalyzeCodeTarget resolveAnalyzeCodeTargetFromMessage(String message,
        ...
        if (focus != null && focus.getPath() != null && !focus.getPath().isBlank()) { // 有最近项目源码文件焦点。
            return new AnalyzeCodeTarget(focus.getPath(), true); // 用焦点文件分析。
        }
        return null; // 无目标时不猜测。
    }
    ...
    private String extractControllerEndpoint(String message) { // 从用户输入中提取接口路径，如 /api/agent/chat。
        if (message == null || message.isBlank() || !message.contains("/")) { // 没有斜杠不会有接口路径。
            return null; // 返回null。
        }
        Matcher matcher = CONTROLLER_ENDPOINT_PATTERN.matcher(message); // 匹配 / 开头的接口路径。
        while (matcher.find()) { // 逐个候选。
            String candidate = matcher.group(1); // 候选接口路径。
            int start = matcher.start(1); // 记录命中起点，用来排除 Tech-Brain-Agent/src/... 里的中间斜杠。
            int end = matcher.end(1); // 记录命中终点，用来排除 /AgentController.java 被截断成 endpoint。
            if (candidate != null && candidate.length() > 1
                    && !candidate.contains(".")
                    && !isEndpointEmbeddedInProjectPath(message, start, end)) { // 排除项目文件路径里的片段。
                return sanitizeEndpointCandidate(candidate); // 返回清理后的接口路径。
            }
        }
        return null; // 没有接口路径。
    }
    ...
    private String sanitizeEndpointCandidate(String candidate) { // 清理自然语言中的 endpoint 候选。
        ...
        while (endpoint.endsWith("，")
                ...
                || endpoint.endsWith("!")
                || endpoint.endsWith("：")
                || endpoint.endsWith(":")) { // 去掉用户句尾标点。
            endpoint = endpoint.substring(0, endpoint.length() - 1); // 删除末尾标点。
        }
        return endpoint; // 返回清理后的 endpoint。
    }
