 ...
 * 用户文件上传返回对象。
 *
 * <p>适用场景：文件上传成功后返回给前端的安全文件基础信息，包含文件 ID、原始文件名、扩展名、MIME 类型、
 * 文件分类、大小、存储类型、访问地址和创建时间。</p>
 * <p>调用链：UserFileServiceImpl.uploadFile 保存文件并写入 user_file 表 -> 转换为 UserFileVO
 * -> UserFileController 通过 Result 返回给前端。</p>
 * <p>边界说明：本 VO 不返回 storagePath，避免暴露服务器真实存储路径；当前 accessUrl 可以为空，后续下载接口再补。</p>
 */
